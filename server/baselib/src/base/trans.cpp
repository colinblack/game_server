...
int CTrans::STOI(const char* szValue)
{
	return szValue == NULL ? 0 : atoi(szValue);
}

int CTrans::STOI(const std::string& sValue)
{
...
const std::string CTrans::ITOS(int iValue, std::streamsize nPrecision, std::streamsize nWide, char chFill, std::ios::fmtflags fmt)
{
	std::string sValue;
...
const std::string CTrans::UTOS(unsigned uValue, std::streamsize nPrecision, std::streamsize nWide, char chFill, std::ios::fmtflags fmt)
{
	std::string sValue;
	std::stringstream strm;
	strm.setf(fmt);
	strm<<std::setprecision(nPrecision)<<std::setw(nWide)<<std::setfill(chFill)<<uValue;
	strm>>sValue;
	return sValue;
}
