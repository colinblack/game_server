...
int CDBCRouteTable::init(char * xml_config)
{
	...
	if( !XMLConf.FindElem("system") ) {
		MIN_UIN =0;
		MAX_UIN =18446744073709551615;
	}else{
		MIN_UIN =strtoull(XMLConf.GetAttrib("minuin").c_str(),NULL,0);
		MAX_UIN =strtoull(XMLConf.GetAttrib("maxuin").c_str(),NULL,0);
	}
	...
	while(XMLConf.FindChildElem("route"))
	{
		...
		if (iRet!=0) 
			warn_log("CDBCRouteTable::init() err:file=%s ", XMLConf.GetChildAttrib("file").c_str());
	}
	return 0;
}
...
int CDBCRouteTable::load_rout(const std::string  & sPath)
{
	...
	if( !XMLConf.FindElem("uin_dbc") ) {
		...
		stHeardInfo.iMod  =strtoull(XMLConf.GetAttrib("mod").c_str(),NULL,0);
		stHeardInfo.iDiv  =strtoull(XMLConf.GetAttrib("div").c_str(),NULL,0);
		stHeardInfo.iMaxnum=strtoull(XMLConf.GetAttrib("maxnum").c_str(),NULL,0);
	...
	{
		UIN_DBC stUinTtc;
		stUinTtc.iBeg_uin =strtoull(XMLConf.GetChildAttrib("begin").c_str(),NULL,0);
		stUinTtc.iEnd_uin =strtoull(XMLConf.GetChildAttrib("end").c_str(),NULL,0);
...
int CDBCRouteTable::get_route(int iTableId, unsigned long long iUin, char ** ip, unsigned& port)
{
	...
	if (!bFlag)
	{
		...
		warn_log("get_route() err:table id=%d ", iTableId);
		return -3;
	}
...
int CDBCRouteTable::get_route(char * sTable,unsigned long long iUin,DBC::Server **m_stServer)
{
	if (iInitSuc !=0)return -1;

	unsigned long long iModUin =0 ;
	vector<HEARD_INFO>::iterator it;
	for (it=m_vstHeardInfo.begin(); it!=m_vstHeardInfo.end();it++)
	{
		if ( strcmp(sTable,it->sTable)==0 )
		{
			...
			if(iUin < it->iMod)
			{
				sErrMsg = string("not found: iTableId =")+CTrans::ITOS(it->iTableId)+" iUin="+CTrans::UTOS(iUin);
				return -4;
			}
			iModUin = (iUin - it->iMod) / it->iDiv;
			break;
	...
	warn_log("get_route() err:table id=%s ", sTable);
	return -4;
};

int CDBCRouteTable::get_route(int iTableId,unsigned long long iUin,DBC::Server ** m_stServer,
                             bool isUseConf,  unsigned pn_MaxNum, char** ip)
{
	if (iInitSuc !=0)return -1;
	...
	for (vector<HEARD_INFO>::iterator it=m_vstHeardInfo.begin(); it!=m_vstHeardInfo.end();it++)
	{
		if ( (it->iTableId)==iTableId )
		{
			...
			//debug_log("found: iTableId=%d,iUin=%u,mod=%llu,div=%llu",iTableId,iUin,it->iMod,it->iDiv);
...
int CDBCRouteTable::get_ip(int iTableId,unsigned long long iUin, string & ps_IP, unsigned int & pn_Port)
{
	ps_IP.clear();
	pn_Port = 0;
...
int CDBCRouteTable::get_route_batch(char *sTable,std::vector<unsigned long long>  iUins, std::vector<BATCH_DBC> vBatchTtc)
{
	//todo
	return -1;
}
...
int CDBCRouteTable::CheckConnectCount(int iTableId, unsigned long long iUin, DBC::Server * mp_stServer,
			 unsigned pn_MaxNum)
{
	if( !mp_stServer) {
		return -1;
