...
int ProtoManager::ProcessLogin(Common::Login* msg)
{
	...
	if(ret)
	{
		info_log("kick login error, uid=%u, fd=%u", msg->uid(), LMI->Getfd());
...
int ProtoManager::ProcessBagPurchase(BagLogic::PurchaseBagReq *msg)
{

	BagLogic::PurchaseBagResp * bagresp = new BagLogic::PurchaseBagResp;

	string errmsg;

	int ret = UserManager::Instance()->ProcessBagAdd(bagresp, errmsg);

	if (ret)
	{
		LogicManager::Instance()->SetErrMsg(errmsg);
	}
	else
	{
		LogicManager::Instance()->SetReplyProtocol(bagresp);
	}
