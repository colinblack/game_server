...
static int checkConfig(const char* confFile){
	...
	const char *onlineLog = pConfig->GetStrVal("LOG","OnlineLog");
	if(!onlineLog)
	{
		fprintf(stderr,"config file must contain [onlineLog] OnlineLog\n");
		return -1;
	}
	INIT_ONLINE_LOG(onlineLog,size*1024*1024,count);
	if(g_pLog == NULL){
		fprintf(stderr,"flow log init failed:%m\n");
		return -1;
	}

	g_pLog->setLogLevel(pConfig->GetIntVal("LOG","LogMode",9));
...
static void sigterm_handler(int signo) {
	if(signo == SIGUSR2){
		if(checkConfig(confFile) == 0){
			LOG_ERROR("reload conf %s success",confFile);
		}
		else{
			LOG_ERROR("reload conf %s failed",confFile);
		}
		return;
	}
