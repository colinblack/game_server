...
WanBaPlatform::WanBaPlatform(): m_pt(-1)
{
}
...
void WanBaPlatform::SetParameter(const map<string, string> &params)
{
	...
	if (it != params.end())
	{
		m_pt = Convert::StringToInt(it->second, -1);
	}
	it = params.find("code");
	if (it != params.end())
	{
	...
	if (m_pf == "test")
	{
		SetPlatformType(PT_TEST);
	}
	else if (m_pf == "wx")
	{
		SetPlatformType(PT_WX);
	}

	if (GetPlatformType() == PT_WB_AND)
	{
		if (m_pt == WAN_BA_OS_AND)
		{
			SetPlatformType(PT_WB_AND);
		}
		else if (m_pt == WAN_BA_OS_IOS)
		{
			SetPlatformType(PT_WB_IOS);
		}
		else
		{
			SetPlatformType(PT_TEST);
	...
	debug_log("pf=%s,pt=%d,pt=%d", m_pf.c_str(), m_pt, GetPlatformByPF());
}
...
int WanBaPlatform::GetUserInfo(OPUserInfo &userInfo, const string &openid, const string &openkey)
{
	...
	if (GetPlatformType() == PT_WX)
	{
		return GetUserInfoWx(userInfo, openid, openkey);
	}
...
int WanBaPlatform::GetUserInfoWx(OPUserInfo &userInfo, const string &openid, const string &openkey)
{
	const string api = "/sns/jscode2session";
	map<string,string> params;
	params["appid"] = m_config["wx_appid"];
	params["secret"] = m_config["wx_appkey"];
	params["js_code"] = m_code;
	params["grant_type"] = m_config["wx_grant_type"];
	string url;
	WanBaPlatform::formatUrl(params, "", api, url, m_config["wx_v3domain"]);
	string response;
	if (!Network::HttpGetRequest(response, url) || response.empty())
	{
		error_log("[http request fail][url=%s]", url.c_str());
		return -1;
	}
	Json::Value value;
	if (!Json::Reader().parse(response, value))
	{
		error_log("[parse resp fail][url=%s,resp=%s]", url.c_str(), response.c_str());
		return -1;
	}
	int ret = 0;
	...
	{
		error_log("[get info fail].response=%s,errcode=%d", response.c_str(), ret);
		return -1;
	}
	string openid_new;
	string session_key;
	...
	{
		error_log("[get info fail].response=%s,openid_new=%s,session_key=%s",
				response.c_str(), openid_new.c_str(), session_key.c_str());
		return -1;
	}
	userInfo.OpenId = openid_new;
	userInfo.OpenKey = session_key;
	return 0;
}
