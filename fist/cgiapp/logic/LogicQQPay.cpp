...
CLogicQQPay::T_ItemMap CLogicQQPay::g_ItemInfo_ios;
CLogicQQPay::T_ItemMap CLogicQQPay::g_ItemInfo_and;
		...
		int os)
{
	...
	if (!bInit)
	{
		...
		{
			fatal_log("[find items info fail][path=%s]", itemFile.c_str());
			ERROR_RETURN_MSG(R_ERROR, "load_item_fail");
		}
		xmlConf.IntoElem();
		if (xmlConf.FindElem("ios"))
		{
			ParseItem(xmlConf, g_ItemInfo_ios);
		}
		if (xmlConf.FindElem("and"))
		{
			ParseItem(xmlConf, g_ItemInfo_and);
		}
		bInit = true;
	}

	if (g_ItemInfo_and.empty() || g_ItemInfo_ios.empty())
	{
	...
	if (os == WAN_BA_OS_AND)
	{
		it = g_ItemInfo_and.find(itemid);
		if (it == g_ItemInfo_and.end())
		{
			error_log("[no item info][itemid=%s]",itemid.c_str());
			ERROR_RETURN_MSG(R_ERR_NO_DATA, "no_item_info");
		}
	}
	else if (os == WAN_BA_OS_IOS)
	{
		it = g_ItemInfo_ios.find(itemid);
		if (it == g_ItemInfo_ios.end())
		{
			error_log("[no item info][itemid=%s]",itemid.c_str());
			ERROR_RETURN_MSG(R_ERR_NO_DATA, "no_item_info");
		}
	...
	{
		error_log("[no item info][itemid=%s,os=%d]",itemid.c_str(),os);
		ERROR_RETURN_MSG(R_ERR_NO_DATA, "no_os_item_info");
	}
		...
		const string &billno)
{
	...
	QQItemInfo itemInfo;
	ret = GetItemInfo(items[0], itemInfo, 0);
	if (ret != 0)
	{
		ERROR_RETURN_MSG(4, "请求参数错误：（payitem） ");
	}
		...
		string &url_params)
{
	...
	if (ret != 0)
		return ret;
	QQItemInfo itemInfo;
	ret = GetItemInfo(itemid, itemInfo, 0);
	if (ret != 0)
		return ret;
...
int CLogicQQPay::ParseItem(CMarkupSTL &xmlConf, T_ItemMap &items)
{
	xmlConf.IntoElem();
	while(xmlConf.FindElem("item"))
	{
		...
		itemTmp.pid = xmlConf.GetAttrib("pid");
