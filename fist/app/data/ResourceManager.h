...
struct SkillInfo
{
	...
	SkillInfo() : id(0),type(0), hurt(0.0), mul(0.0), p_c(0), p_a(0)
	{
	}
};
...
struct OfflineResourceItem
{
	...
	void Parse(const Offline::Skill &msg, SkillInfo &skill)
	{
		...
		if(msg.has_hurt())
			skill.hurt = msg.hurt();
		if(msg.has_multiple())
			skill.mul = msg.multiple();
		if(msg.has_initweight())
			skill.p_c = msg.initweight();
		if(msg.has_useraddweight())
			skill.p_a = msg.useraddweight();
	}
	...
	void Serialize(Offline::Skill *msg, SkillInfo &skill)
	{
		...
		msg->set_multiple(skill.mul);
		msg->set_initweight(skill.p_c);
		msg->set_useraddweight(skill.p_a);
	}
