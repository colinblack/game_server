...
bool LogicManager::Initialize(){
	...
	for(vector<BattleSingleton*>::iterator it=m_battleManager.begin();it!=m_battleManager.end();++it)
	{
		try
		{
			...
			if(ret)
			{
				error_log("%s, BattleSingleton OnInit error!", typeid(**it).name());
				return false;
			}
		}
		catch(const std::exception&)
...
void LogicManager::RegProto()
{
	...
	//跨服踢馆发给user
	dispatcher.registerMessageCallback<ProtoBattle::ProtoCSBattleResultReq>(ProtoManager::ProcessNoReplyNoUID<ProtoBattle::ProtoCSBattleResultReq, LogicBattleManager>);
	//推荐武馆
	...
	//开始
	dispatcher.registerMessageCallback<ProtoChampionship::ProtoStartBattleReq>(ProtoManager::ProcessNoReply<ProtoChampionship::ProtoStartBattleReq, LogicChampionshipManager>);
...
void LogicManager::RegMemoryManager()
{
	...
	m_memoryManager.push_back(ChampionshipManager::Instance());
