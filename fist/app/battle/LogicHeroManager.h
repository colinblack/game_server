...
public:
	...
	void GetSkill(unsigned uid, unsigned id, Skill skill)
	{
		vector<char> s;
		DBCHeroWrap heroWrap(uid, id);
		heroWrap.GetSkill(s);
		...
		for(int i = 0; i < e_skill_max; ++i)
		{
			level = s[i];
			skill[i].type 	= i;
			skill[i].id 		= hero_cfg.skill(i).id();
			skill[i].mul		= hero_cfg.skill(i).skillinfo(level-1).multiple();
			skill[i].hurt 		= hero_cfg.skill(i).skillinfo(level-1).hurtvalue();
			skill[i].p_c	 	= LogicHeroManager::initWeight[i];
			skill[i].p_a		= AddWeightCfgWarp().GetPlayerAddWeightBylevel(level).weight(i); //玩家等级增加权重
		}
	}
	//掉落龙珠, 玩法改变已弃用
