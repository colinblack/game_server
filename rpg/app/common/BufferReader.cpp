...
bool CBufferReader::GetInt64(int64_t &value) {
	uint32_t t = 0, e = 0;
	if (m_bSwapByte) {
		if (!GetUInt32(e)) {
			return false;
	...
	if (t % 2 == 0) {
		value = static_cast<int64_t>(t) / 2 + 0x80000000 * static_cast<int64_t>(e);
	} else {
		value = -((static_cast<int64_t>(t) - 1) / 2 + 0x80000000 * static_cast<int64_t>(e) + 1);
	}
	return true;
}

bool CBufferReader::GetUInt64(uint64_t &value) {
	uint32_t t = 0, e = 0;
	if (m_bSwapByte) {
		if (!GetUInt32(e)) {
			return false;
	...
	value = static_cast<uint64_t>(t) + static_cast<uint64_t>(e) * 0x100000000;
	return true;
}
...
bool CBufferReader::GetFloat(float &value)
{
	...
	if(!m_pBuffer->GetData((byte *)&value, sizeof(float), m_index))
	{
		return false;
	}
...
bool CBufferReader::GetDouble(double &value)
{
	...
	if(!m_pBuffer->GetData((byte *)&value, sizeof(double), m_index))
	{
		return false;
	}
...
//	}
	value.resize(size);
	if(size > 0 && !m_pBuffer->GetData((byte *)const_cast<char*>(value.c_str()), size, m_index))
	{
		return false;
	}
	m_index += size;
...
bool CBufferReader::GetUVar32(uint32_t &value) {
	byte flag = 0;
	if (!GetByte(flag)) {
		return false;
	}
...
bool CBufferReader::GetBool(bool &value) {
	byte flag = 0;
	if (!GetByte(flag)) {
		return false;
	}
