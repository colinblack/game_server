...
namespace Convert
{
	...
	float StringToFloat(const string &s, float defaultValue = 0);
	double StringToDouble(const string &s, double defaultValue = 0);
	bool StringToBool(const string &s, double defaultValue = false);
	...
	bool JsonToMsg(Message *msg, const Json::Value &json);
};
