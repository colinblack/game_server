...
string Config::GetPath(const string &path) {
	string value = GetValue("server_path");
	if (value.empty()) {
		return value;
	}
	string sid = GetValue("server_id");
	if (!path.empty()) {
		if (value[value.size() - 1] == '/') {
			value.append("s").append(sid).append("/").append(path);
		} else {
			value.append("/s").append(sid).append("/").append(path);
		}
	}
...
string Config::GetAllServerPath(const string &path) {
	string value = GetValue("server_path");
	if (value.empty()) {
		return value;
	}
