...
CPollerObject::CPollerObject(CPollerUnit *o, int fd) :
		netfd(fd), ownerUnit(o), newEvents(0), oldEvents(0), epslot(NULL) {
}
...
int CPollerObject::AttachPoller(CPollerUnit *unit) {
	...
	if (epslot == NULL) {
		...
		debug_log("Attach poller succ:%d", netfd);
		return 0;
	}
	debug_log("Attach poller succ:%d", netfd);
	return ApplyEvents();
...
int CPollerUnit::InitializePollerUnit(void) {
	...
	if (!pollerTable) {
		error_log("calloc failed, num=%d, %m", maxPollers);
		return -1;
	}
	...
	if (!ep_events) {
		error_log("malloc failed, %m");
		return -1;
	}
	if ((epfd = epoll_create(maxPollers)) == -1) {
		warn_log("epoll_create failed, %m");
		return -1;
	}
...
inline int CPollerUnit::VerifyEvents(struct epoll_event *ev) {
	...
	if (pollerTable[idx].poller == NULL || pollerTable[idx].freeList != NULL) {
		warn_log("receive invalid epoll event. idx=%d seq=%d poller=%p freelist=%p event=%x", idx, (int)EPOLL_DATA_SEQ(ev), pollerTable[idx].poller, pollerTable[idx].freeList, ev->events);
		return -1;
	}
	return 0;
}
...
struct CEpollSlot *CPollerUnit::AllocEpollSlot() {
	...
	if (0 == p) {
		warn_log("no free epoll slot, usedPollers=%d", usedPollers);
		return NULL;
	}
	usedPollers++;
...
int CPollerUnit::Epctl(int op, int fd, struct epoll_event *events) {
	if (epoll_ctl(epfd, op, fd, events) == -1) {
		warn_log("epoll_ctl error, epfd=%d, fd=%d", epfd, fd);
		return -1;
	}
	return 0;
}
...
void CPollerUnit::ProcessPollerEvents(void) {
	for (int i = 0; i < nrEvents; i++) {
		if (VerifyEvents(ep_events + i) == -1) {
			warn_log("VerifyEvents failed, ep_events[%d].data.u64 = %llu", i, (unsigned long long )ep_events[i].data.u64);
			continue;
		}
