...
CTcpServer::CTcpServer() :
		m_bRunning(false), m_listenCount(0), m_maxConn(0), m_pChannels(NULL), m_lastFreeChannelId(0), m_fdEpoll(-1) {
}
...
bool CTcpServer::Initialize(const vector<CInternetAddress> &listenList, int maxConn) {
	...
	for (int i = 0; i < m_listenCount; i++) {
		if (!m_pChannels[i].CreateServer(i, m_listenList[i])) {
			fatal_log("[CreateServer fail][error=%d,ip=%s,port=%d]", m_pChannels[i].GetLastError(), m_listenList[i].IP.c_str(), (int )m_listenList[i].Port);
			close(m_fdEpoll);
			return false;
		}
					...
					if ((events[i].events & (EPOLLERR | EPOLLHUP)) != 0) {
						...
						if (acceptChannelId < 0) {
							...
							if (m_pChannels[acceptChannelId].Accept(acceptChannelId, *pChannel)) {
								...
								error_log("[socket accept error][error=%d,id=%d,fd=%d]", m_pChannels[acceptChannelId].GetLastError(), acceptChannelId, m_pChannels[acceptChannelId].GetSocket());
							}
						}
					}
				} else {
...
bool CTcpServer::SendData(CFirePacket *pHead, IPacketSend *pBody) {
	...
	if (m_pChannels[pHead->ChannelId].IsClosed()) {
		error_log("[channel_closed][channelid=%d,fd=%u,time=%u,cmd=%u,body=%u]", pHead->ChannelId, pHead->fd, (int )pHead->time, pHead->cmd, pHead->body.GetSize());
		return false;
	}
	if (pBody != NULL) {
...
void CTcpServer::OnChannelReceive(CTcpChannel *pChannel) {
	...
	do {
		...
		if (decodeSize > 0) {
			if (!decodeSuccess) {
				error_log("[Decode fail][channelId=%d,size=%u,packet=%s]", pChannel->GetChannelId(), pChannel->GetReceiveData()->GetSize(), pChannel->GetReceiveData()->ToString().c_str());
				pChannel->Close();
				decodeSize = 0;
				break;
			}
...
bool CTcpServer::SendData(CFirePacket* packet) {
	...
	if (m_pChannels[packet->ChannelId].IsClosed()) {
		error_log("[channel_closed][channelid=%d,fd=%u,time=%u,cmd=%u,body=%u]", packet->ChannelId, packet->fd, (int )packet->time, packet->cmd, packet->body.GetSize());
		return false;
	}
