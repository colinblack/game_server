...
public:
	...
	// 将equipManager中的背包使用道具部分提取出来 封装成一个函数
	bool BagUseTitleItem(uint32_t uid ,uint32_t equipId);
...
public:
	int Sync(const UserCache &cache, uint32_t cmd, msgs::SPlayerTitleInfo *resp); // 服务器主动推送的消息
