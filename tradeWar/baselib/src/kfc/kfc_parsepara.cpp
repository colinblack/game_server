...
kfc::parse_formaterr::parse_formaterr(const string & s):runtime_error(s){}

unsigned parse_x2u(const string &s)
{
	size_t len = s.length();
	size_t idx=0;
	...
	if(len==0 || len>8) {
		throw kfc::parse_formaterr(string("CCgiPara::Decode: length is invalid:")+s);
	}
	for (size_t i = idx; i < s.length(); i++){
		if (!isxdigit(s.c_str()[i])) throw kfc::parse_formaterr(string("CCgiPara::Decode: not xdigit:")+s);
	}

	return strtoul(s.c_str()+idx,NULL,16);
}

kfc::CCgiPara::CCgiPara(){}
kfc::CCgiPara::~CCgiPara(){}
// _map内存放的有两种格式,string和binary string
inline bool kfc::CCgiPara::isBinaryString(const string &s)
{
	int len = s.length();
	if(len>8) {
...
inline string kfc::CCgiPara::FormatC2I(const string &s)
{
	string out;
	unsigned char c;
	bool isbin = false;
...
// 内部格式转换到cgi参数格式
// 2004-11-10,改汉字不转码
inline string kfc::CCgiPara::FormatI2C(const string &s)
{
	// 如果是binary string格式
...
void kfc::CCgiPara::Decode(const string & s)throw(parse_formaterr)
{
	Decode(s.c_str());
}
void kfc::CCgiPara::Decode(const char *s)throw(parse_formaterr)
{
	string name;
	string value;
	bool find;
...
// binary 到内部格式
string kfc::CCgiPara::b2s(const char * b, size_t len)
{
	string out;
	char sTmp[4]={0};
...
string kfc::CCgiPara::b2sx(const char * b, size_t len)
{
	string out;
	char sTmp[4]={0};
...
// 内部格式到bianry
size_t kfc::CCgiPara::s2b(const string & s, char * b, size_t maxlen)
{
	size_t len = s.length();
	if(isBinaryString(s)) {
...
kfc::CSpacePara::CSpacePara()
{
	_split = " ,#\t\r\n";	
}
kfc::CSpacePara::CSpacePara(const string & s):_split(s)
{
}
kfc::CSpacePara::~CSpacePara(){}

// 2004-11-10,不再支持'\'转义
inline bool kfc::CSpacePara::isplit(unsigned char c,unsigned char c2)
{
...
void kfc::CSpacePara::Decode(const string & s)
{
	Decode(s.c_str());
}
void kfc::CSpacePara::Decode(const char *s)
{
	string value;
	char *p=(char *)s;
	char *p1;
...
void kfc::CSpacePara::DecodeStrict(const string & s)
{
	DecodeStrict(s.c_str());
}
void kfc::CSpacePara::DecodeStrict(const char *s)
{
	string value;
	char *p=(char *)s;
	char *p1;
