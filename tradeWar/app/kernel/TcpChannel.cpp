...
bool CTcpChannel::RealSend(){
	...
	{
		debug_log("send data size = %d, left size = %u, time = %u",byteSent,byteLeft,m_lastActiveTime);
	}
//		byteLeft -= byteSent;
...
//	debug_log("send data\n%u",pBuffer->GetSize());
	...
	while(byteLeft > 0)
	{
		...
		else if(byteSent < 0)
		{
			...
			{
				//系统发送缓冲区已满，放进滞留缓冲区
				debug_log("send buff full length= %d",pBuffer->GetSize() - sendPos);
...
{
	debug_log("channel %d closed",m_channelId);
