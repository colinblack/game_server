...
CChatPacket::CChatPacket(const CChatPacket &other)
{
	ChannelId = other.ChannelId,
	PacketType = other.PacketType;
	PacketFlag = other.PacketFlag;
	From = other.From;
	To = other.To;
	...
	ProtocolFlag = other.ProtocolFlag;
	ProtocolResult = other.ProtocolResult;
	m_decodeSize = other.m_decodeSize;
	ProtocolBody = other.ProtocolBody;
}


static byte s_baReserveData[8] = {0};

bool CChatPacket::Encode(IBuffer *pBuffer)
{
	if(pBuffer == NULL)
	{
		return false;
	...
	uint32_t packetLength = PACKET_HEADER_SIZE1 + ProtocolBody.GetSize() + sizeof(byte);
	...
	PACKET_ENCODE(Byte, PACKET_HEADER_SIZE1);
	PACKET_ENCODE(Byte, 0);
...
bool CChatPacket::Decode(IBuffer *pBuffer)
{
	m_decodeSize = 0;
	if(pBuffer == NULL)
	{
	...
	if(packetLength < PACKET_HEADER_SIZE1 + sizeof(byte))
	{
		return false;
	}
	byte head = 0;
	...
	if(bodyOffset == 0)
	{
		bodyOffset = PACKET_HEADER_SIZE1;
	}
	else if(bodyOffset < PACKET_HEADER_SIZE1)
	{
		return false;
	}
	...
	if(bodyOffset != 0)
	{
		PACKET_DECODE_SKIP(bodyOffset - PACKET_HEADER_SIZE1);
...
uint32_t CChatPacket::GetDecodeSize()
{
	return m_decodeSize;
}
