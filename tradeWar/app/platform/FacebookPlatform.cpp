...
int CFacebookPlatform::GetUserInfo(OPUserInfo &userInfo, const string &openid, const string &openkey, const string &pf, const string &userip) {
	string url;
	string response;
	String::Format(url, URL_FB_GET_USER_INFO, openkey.c_str());
	if (!Network::HttpGetRequest(response, url)) {
		m_errorMessage = response;
		error_log("[http request fail][reason=HttpGetRequest,openid=%s,errmsg=%s]", openid.c_str(), m_errorMessage.c_str());
		return -1;
	}
	Json::Value jsonUserInfo;
	Json::Reader reader;
	if (!reader.parse(response, jsonUserInfo) || !jsonUserInfo.isObject()) {
		m_errorMessage = "json_parse_fail";
		error_log("[json parse fail][reason=JsonParse,openid=%s,errmsg=%s,response=%s]", openid.c_str(), m_errorMessage.c_str(), response.c_str());
		return -1;
	}
	if (!jsonUserInfo.isMember("id")) {
		m_errorMessage = "query_fail";
		error_log("[data error][reason=QueryFail,openid=%s,errmsg=%s,response=%s]", openid.c_str(), m_errorMessage.c_str(), response.c_str());
		return -1;
	}
...
int CFacebookPlatform::GetAppFriendList(OPFriendList &friendList, const string &openid, const string &openkey, const string &pf, const string &userip) {
	//string fql;
	...
	if (!parseSuccess) {
		m_errorMessage = "json_parse_fail";
		error_log("[json parse fail][reason=XmlParse,openid=%s,response=%s]", openid.c_str(), response.c_str());
		return -1;
	}
...
int CFacebookPlatform::GetOtherUserInfo(OPUserInfo userInfo, const string &queryOpenId, const string &openid, const string &openkey) {
	...
	if (!Network::HttpGetRequest(response, url)) {
		m_errorMessage = response;
		error_log("[http request fail][reason=HttpGetRequest,openid=%s,errmsg=%s]", queryOpenId.c_str(), m_errorMessage.c_str());
		return -1;
	}
	Json::Value jsonUserInfo;
	Json::Reader reader;
	if (!reader.parse(response, jsonUserInfo) || !jsonUserInfo.isObject()) {
		m_errorMessage = "json_parse_fail";
		error_log("[json parse fail][reason=JsonParse,openid=%s,errmsg=%s,response=%s]", queryOpenId.c_str(), m_errorMessage.c_str(), response.c_str());
		return -1;
	}
	if (!jsonUserInfo.isMember("id")) {
		m_errorMessage = "query_fail";
		error_log("[data error][reason=QueryFail,openid=%s,errmsg=%s,response=%s]", queryOpenId.c_str(), m_errorMessage.c_str(), response.c_str());
		return -1;
	}
...
int CFacebookPlatform::SendFeed(const string &openid, const string &openkey, const FacebookFeed &feed) {
	string url;
	string response;
	String::Format(url, "https://graph.facebook.com/%s/feed?access_token=%s", feed.to.empty() ? "me" : feed.to.c_str(), openkey.c_str());
	string params;
	String::Format(params, "message=%s&picture=%s&link=%s&name=%s&caption=%s&description=%s&source=%s&actions=%s", Crypt::UrlEncode(feed.message).c_str(), feed.picture.c_str(), Crypt::UrlEncode(feed.link).c_str(), Crypt::UrlEncode(feed.name).c_str(), Crypt::UrlEncode(feed.caption).c_str(), Crypt::UrlEncode(feed.description).c_str(), Crypt::UrlEncode(feed.source).c_str(), Crypt::UrlEncode(feed.actions).c_str());
	if (!Network::HttpPostRequest(response, url, params)) {
		m_errorMessage = response;
		error_log("[http request fail][reason=HttpPostRequest,openid=%s,errmsg=%s]", openid.c_str(), m_errorMessage.c_str());
		return -1;
	}
