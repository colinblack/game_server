...
bool CShareMemory::Create(const char *name, int size)
{
	if(name == NULL)
	{
		return false;
...
bool CShareMemory::Open(const char *name, int size)
{
	if(name == NULL)
	{
		return false;
	...
	if(fd < 0)
	{
		LogError("open <%s> O_RDWR Fail [%d] reanson=%m", name, errno);
		return false;
	}
	...
	if(!m_lock.CreateOrOpen(lockName.c_str()))
	{
...
bool CShareMemory::CreateOrOpen(const char *name, int size)
{
	if(!Open(name, size))
	{
		if(!Create(name, size))
