...
const Order& Order::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ConfigOrder_2eproto();  return *default_instance_;
}
...
bool Base::IsInitialized() const {
  ...
  for (int i = 0; i < ordersolt_size(); i++) {
    if (!this->ordersolt(i).IsInitialized()) return false;
  }
  if (has_orderbonus()) {
