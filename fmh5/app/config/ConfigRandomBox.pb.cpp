...
const FreeRandomBoxCPP& FreeRandomBoxCPP::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ConfigRandomBox_2eproto();  return *default_instance_;
}
...
bool RandomBoxCPP::IsInitialized() const {
  ...
  for (int i = 0; i < boxtype_size(); i++) {
    if (!this->boxtype(i).IsInitialized()) return false;
  }
  return true;
}
...
bool RandomBoxInfo::IsInitialized() const {
  ...
  for (int i = 0; i < freeboxgift_size(); i++) {
    if (!this->freeboxgift(i).IsInitialized()) return false;
  }
  for (int i = 0; i < chargeboxgift_size(); i++) {
    if (!this->chargeboxgift(i).IsInitialized()) return false;
  }
  return true;
}
