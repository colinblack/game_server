...
const VIPGiftCPP& VIPGiftCPP::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ConfigVIP_2eproto();  return *default_instance_;
}
...
bool VIPBase::IsInitialized() const {
  ...
  for (int i = 0; i < vipgift_size(); i++) {
    if (!this->vipgift(i).IsInitialized()) return false;
  }
  return true;
}
