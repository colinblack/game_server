...
const AdReardItemLibCPP& AdReardItemLibCPP::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ConfigAccessAd_2eproto();  return *default_instance_;
}
...
bool AdRewardCPP::IsInitialized() const {
  ...
  for (int i = 0; i < itemlib_size(); i++) {
    if (!this->itemlib(i).IsInitialized()) return false;
  }
  return true;
}
...
bool ViewAdCPP::IsInitialized() const {
  ...
  for (int i = 0; i < reward_item_lib_size(); i++) {
    if (!this->reward_item_lib(i).IsInitialized()) return false;
  }
  return true;
}
