...
const RotaryTableCPP& RotaryTableCPP::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ConfigRotaryTable_2eproto();  return *default_instance_;
}
...
bool RotaryTableCfg::IsInitialized() const {
  ...
  for (int i = 0; i < gridcommon_size(); i++) {
    if (!this->gridcommon(i).IsInitialized()) return false;
  }
  if (has_griditeminfo()) {
