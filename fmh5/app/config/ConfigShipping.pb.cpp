...
const BoxItems& BoxItems::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ConfigShipping_2eproto();  return *default_instance_;
}
...
bool ShippingItem::IsInitialized() const {
  ...
  for (int i = 0; i < level_items_size(); i++) {
    if (!this->level_items(i).IsInitialized()) return false;
  }
  return true;
}
