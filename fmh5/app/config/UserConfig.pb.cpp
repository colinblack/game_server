...
bool DailyShareGifts::IsInitialized() const {
  ...
  for (int i = 0; i < extra_size(); i++) {
    if (!this->extra(i).IsInitialized()) return false;
  }
  return true;
}
...
void XMUserRewardCfg::SharedCtor() {
  ...
  firstmailmsg_ = const_cast< ::std::string*>(&::google::protobuf::internal::kEmptyString);
  firstchargemailmsg_ = const_cast< ::std::string*>(&::google::protobuf::internal::kEmptyString);
  ::memset(_has_bits_, 0, sizeof(_has_bits_));
}
...
void XMUserRewardCfg::SharedDtor() {
  ...
  if (firstmailmsg_ != &::google::protobuf::internal::kEmptyString) {
    delete firstmailmsg_;
  }
  if (firstchargemailmsg_ != &::google::protobuf::internal::kEmptyString) {
...
bool XMUserRewardCfg::IsInitialized() const {
  ...
  for (int i = 0; i < rewarddiamondcnt_size(); i++) {
    if (!this->rewarddiamondcnt(i).IsInitialized()) return false;
  }
  return true;
}
...
bool User::IsInitialized() const {
  ...
  for (int i = 0; i < criticalpoint_size(); i++) {
    if (!this->criticalpoint(i).IsInitialized()) return false;
  }
  if (has_sharegift()) {
    if (!this->sharegift().IsInitialized()) return false;
  }
  for (int i = 0; i < coinpurchase_size(); i++) {
    if (!this->coinpurchase(i).IsInitialized()) return false;
  }
  for (int i = 0; i < signin_size(); i++) {
    if (!this->signin(i).IsInitialized()) return false;
  }
  if (has_fairyspeedup()) {
    if (!this->fairyspeedup().IsInitialized()) return false;
  }
  for (int i = 0; i < newuserguideshare_size(); i++) {
    if (!this->newuserguideshare(i).IsInitialized()) return false;
  }
  if (has_xmuserreward()) {
