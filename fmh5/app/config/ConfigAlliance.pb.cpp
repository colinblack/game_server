...
const TypeStorage& TypeStorage::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ConfigAlliance_2eproto();  return *default_instance_;
}
...
bool Donation::IsInitialized() const {
  ...
  for (int i = 0; i < type_storge_size(); i++) {
    if (!this->type_storge(i).IsInitialized()) return false;
  }
  return true;
}
