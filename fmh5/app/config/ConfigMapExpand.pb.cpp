...
const UnLockLandCPP& UnLockLandCPP::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ConfigMapExpand_2eproto();  return *default_instance_;
}
...
bool MapExpandCPP::IsInitialized() const {
  ...
  for (int i = 0; i < land_size(); i++) {
    if (!this->land(i).IsInitialized()) return false;
  }
  return true;
}
...
bool MapExpandInfo::IsInitialized() const {
  ...
  for (int i = 0; i < unlockcost_size(); i++) {
    if (!this->unlockcost(i).IsInitialized()) return false;
  }
  return true;
}
