...
const PetResidenceCPP& PetResidenceCPP::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ConfigPet_2eproto();  return *default_instance_;
}
...
bool PetGardenCfg::IsInitialized() const {
  ...
  for (int i = 0; i < pet_feed_reward_size(); i++) {
    if (!this->pet_feed_reward(i).IsInitialized()) return false;
  }
  for (int i = 0; i < pet_tease_reward_size(); i++) {
    if (!this->pet_tease_reward(i).IsInitialized()) return false;
  }
  return true;
}
