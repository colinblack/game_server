...
const RaceGradeRank& RaceGradeRank::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ConfigAllianceRace_2eproto();  return *default_instance_;
}
...
bool RaceTaskStorageItem::IsInitialized() const {
  ...
  for (int i = 0; i < cond_size(); i++) {
    if (!this->cond(i).IsInitialized()) return false;
  }
  return true;
}
...
bool RaceTask::IsInitialized() const {
  ...
  for (int i = 0; i < chance_size(); i++) {
    if (!this->chance(i).IsInitialized()) return false;
  }
...
bool RaceReward::IsInitialized() const {
  ...
  for (int i = 0; i < grade_reward_size(); i++) {
    if (!this->grade_reward(i).IsInitialized()) return false;
  }
...
bool RaceInfo::IsInitialized() const {
  ...
  for (int i = 0; i < grade_size(); i++) {
    if (!this->grade(i).IsInitialized()) return false;
  }
  if (has_watch_ad_reward()) {
