...
const DynamicInfo& DynamicInfo::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ProtoDynamicInfo_2eproto();  return *default_instance_;
}
...
bool GetDynamicInfoResp::IsInitialized() const {
  ...
  for (int i = 0; i < arraydyinfo_size(); i++) {
    if (!this->arraydyinfo(i).IsInitialized()) return false;
  }
  return true;
}
