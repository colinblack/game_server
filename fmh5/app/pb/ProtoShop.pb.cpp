...
const ShopCPP& ShopCPP::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ProtoShop_2eproto();  return *default_instance_;
}
...
bool VisitOtherShopResp::IsInitialized() const {
  ...
  for (int i = 0; i < othshop_size(); i++) {
    if (!this->othshop(i).IsInitialized()) return false;
  }
  return true;
}
