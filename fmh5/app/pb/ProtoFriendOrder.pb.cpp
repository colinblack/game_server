...
const FriendOrderInfo& FriendOrderInfo::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ProtoFriendOrder_2eproto();  return *default_instance_;
}
...
bool GetFriendOrderResp::IsInitialized() const {
  ...
  for (int i = 0; i < arrayfoinfo_size(); i++) {
    if (!this->arrayfoinfo(i).IsInitialized()) return false;
  }
  return true;
}
