...
bool GetAllFriendsResp::IsInitialized() const {
  ...
  for (int i = 0; i < concerns_size(); i++) {
    if (!this->concerns(i).IsInitialized()) return false;
  }
  for (int i = 0; i < fans_size(); i++) {
    if (!this->fans(i).IsInitialized()) return false;
  }
  for (int i = 0; i < helpers_size(); i++) {
    if (!this->helpers(i).IsInitialized()) return false;
  }
  return true;
}
...
bool CSGetFriendInfoResp::IsInitialized() const {
  ...
  for (int i = 0; i < folk_size(); i++) {
    if (!this->folk(i).IsInitialized()) return false;
  }
  return true;
}
