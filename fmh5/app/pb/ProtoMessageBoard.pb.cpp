...
const MessageInfo& MessageInfo::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ProtoMessageBoard_2eproto();  return *default_instance_;
}
...
bool GetMessageBoardResp::IsInitialized() const {
  ...
  for (int i = 0; i < arraymsgbrd_size(); i++) {
    if (!this->arraymsgbrd(i).IsInitialized()) return false;
  }
  return true;
}
...
bool GetFeedbackResp::IsInitialized() const {
  ...
  for (int i = 0; i < arrayfeedback_size(); i++) {
    if (!this->arrayfeedback(i).IsInitialized()) return false;
  }
  return true;
}
