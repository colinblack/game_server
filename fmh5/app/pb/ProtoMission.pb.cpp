...
const MissionCPP& MissionCPP::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ProtoMission_2eproto();  return *default_instance_;
}
...
bool GetCurMissionResp::IsInitialized() const {
  ...
  for (int i = 0; i < misssion_size(); i++) {
    if (!this->misssion(i).IsInitialized()) return false;
  }
  return true;
}
