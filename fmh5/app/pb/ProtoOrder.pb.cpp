...
const NeedCost& NeedCost::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ProtoOrder_2eproto();  return *default_instance_;
}
...
bool OrderCPP::IsInitialized() const {
  ...
  for (int i = 0; i < needcost_size(); i++) {
    if (!this->needcost(i).IsInitialized()) return false;
  }
  return true;
}
...
bool OrderResp::IsInitialized() const {
  ...
  for (int i = 0; i < order_size(); i++) {
    if (!this->order(i).IsInitialized()) return false;
  }
  return true;
}
