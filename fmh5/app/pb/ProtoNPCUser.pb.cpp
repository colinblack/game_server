...
const Base& Base::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ProtoNPCUser_2eproto();  return *default_instance_;
}
...
bool FriendlyTreeCPP::IsInitialized() const {
  ...
  for (int i = 0; i < basictreeinfo_size(); i++) {
    if (!this->basictreeinfo(i).IsInitialized()) return false;
  }
  if (has_statustreeinfo()) {
