...
const AdInfoCPP& AdInfoCPP::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ProtoAdvertise_2eproto();  return *default_instance_;
}
...
bool GetAdInfoResp::IsInitialized() const {
  ...
  for (int i = 0; i < adinfo_size(); i++) {
    if (!this->adinfo(i).IsInitialized()) return false;
  }
  return true;
}
