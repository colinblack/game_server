...
const RotaryTableCPP& RotaryTableCPP::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ProtoRotaryTable_2eproto();  return *default_instance_;
}
...
bool GetRotaryTableInfoResp::IsInitialized() const {
  ...
  for (int i = 0; i < rotarytable_size(); i++) {
    if (!this->rotarytable(i).IsInitialized()) return false;
  }
  if (has_drawinfo()) {
