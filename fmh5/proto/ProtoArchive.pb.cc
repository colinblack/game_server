...
const UserData& UserData::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_ProtoArchive_2eproto();  return *default_instance_;
}
...
bool UserData::IsInitialized() const {
  ...
  for (int i = 0; i < charges_size(); i++) {
    if (!this->charges(i).IsInitialized()) return false;
  }
  for (int i = 0; i < builds_size(); i++) {
    if (!this->builds(i).IsInitialized()) return false;
  }
  for (int i = 0; i < equipstar_size(); i++) {
    if (!this->equipstar(i).IsInitialized()) return false;
  }
  ...
  for (int i = 0; i < mission_size(); i++) {
    if (!this->mission(i).IsInitialized()) return false;
  }
  for (int i = 0; i < activity_size(); i++) {
    if (!this->activity(i).IsInitialized()) return false;
  }
  for (int i = 0; i < maildog_size(); i++) {
    if (!this->maildog(i).IsInitialized()) return false;
  }
  for (int i = 0; i < friendlytree_size(); i++) {
    if (!this->friendlytree(i).IsInitialized()) return false;
  }
  for (int i = 0; i < shopsellcoin_size(); i++) {
    if (!this->shopsellcoin(i).IsInitialized()) return false;
  }
  for (int i = 0; i < friendworker_size(); i++) {
    if (!this->friendworker(i).IsInitialized()) return false;
  }
