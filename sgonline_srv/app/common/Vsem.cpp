...
bool CVsem::Create(const char *name, int sem, unsigned vnum) {
	_sem = sem;
...
bool CVsem::CreateOrOpen(const char *name, int sem, unsigned vnum) {
	_sem = sem;

	if (!Open(name, sem)) {
		if (!Create(name, sem, vnum))
			return false;
	}
...
bool CVsem::Lock(int vnum) {
	...
	if (semop(m_sem, &sb, 1) == -1) {
		LogError("semop Fail [%d][%d]", errno, _sem);
		return false;
	}
