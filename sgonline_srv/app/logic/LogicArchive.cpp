...
int CLogicArchive::Load(unsigned uid, unsigned uidBy, const string &type,
	...
	{	//加载别人的数据
		...
		else
		{
			...
			{
				if (CLogicUser::GetFlag(user.bit_info, UF_BE_ATTACKED)
						&& user.last_save_uid != uidBy && IsBeingAttacked(user.last_breath_time))
				{
					LOGIC_ERROR_RETURN_MSG("being_attacked");
				}
			}
	...
	if (isMainPos)
	{
		...
		*/
		...
		if (data.isMember("tech")) {
			...
			tempReader.parse(user.user_tech, OldUserTech);

			if (data["tech"]["godE"].isObject()) {
				if (data["tech"]["godE"]["qlE"].isArray()) {
					if (OldUserTech["godE"]["qlE"].isArray()) {
						int iold = OldUserTech["godE"]["qlE"][0u].asInt();
						int ichg = data["tech"]["godE"]["qlE"][0u].asInt();
						int ires = ichg + iold;
						if(ires < 0){
							error_log("[resource_error][uid=%u,userid=%u]", uidBy, uid);
							LOGIC_ERROR_RETURN_MSG("resource_error");
						}
						data["tech"]["godE"]["qlE"][0u] = ires;
						if(ichg != 0)
							RESOURCE_LOG("uid=%u,chg=%d,qlE=%d", uid, ichg, ires);
					}
				}
			}
			if(data["tech"].size())
		...
		if (ret != 0)
			return ret;
		RESOURCE_LOG(
				"[data change][uid=%u,gcbaschg=%d,gcbase=%u,gcbuychg=%d,gcbuy=%u,r1chg=%d,r1=%u,r2chg=%d,r2=%u,r3chg=%d,r3=%u,r4chg=%d,r4=%u,r5chg=%d,r5=%u,addprosper=%d,prosper=%u,addbs=%d,bs=%u]",
...
int CLogicArchive::ProcessAttackInfo(unsigned uid,
	...
	user.point += exp;
	Json::GetInt(attackinfo, "level", user.level);
	int gcbaschg = 0, gcbuychg = 0, addprosper = 0, addbs = 0, itemp = 0;
	...
	if (attackinfo.isMember("tech"))
	{
		...
		if (attackinfo["tech"]["godE"].isObject()) {
			if (attackinfo["tech"]["godE"]["qlE"].isArray()) {
				if (OldUserTech["godE"]["qlE"].isArray()) {
					int iold = OldUserTech["godE"]["qlE"][0u].asInt();
					int ichg = attackinfo["tech"]["godE"]["qlE"][0u].asInt();
					int ires = ichg + iold;
					if(ires < 0){
						error_log("[resource_error][uid=%u]", uid);
						LOGIC_ERROR_RETURN_MSG("resource_error");
					}
					attackinfo["tech"]["godE"]["qlE"][0u] = ires;
					if(ichg != 0)
						RESOURCE_LOG("uid=%u,chg=%d,qlE=%d", uid, ichg, ires);
				}
			}
		}
		if(attackinfo["tech"].size())
	...
	if (ret != 0)
	return ret;
	RESOURCE_LOG("[data change][uid=%u,gcbaschg=%d,gcbase=%u,gcbuychg=%d,gcbuy=%u,r1chg=%d,r1=%u,r2chg=%d,r2=%u,r3chg=%d,r3=%u,r4chg=%d,r4=%u,r5chg=%d,r5=%u,addprosper=%d,prosper=%u,addbs=%d,bs=%u]",
			uid,gcbaschg,user.gcbase,gcbuychg,user.gcbuy,r1chg,user.r1,r2chg,user.r2,r3chg,user.r3,r4chg,user.r4,r5chg,user.r5,addprosper,user.prosper,addbs,user.battle_spirits);
	...
	if (updateinfo.isMember("tech"))
	{
		...
		if (updateinfo["tech"]["godE"].isObject()) {
			if (updateinfo["tech"]["godE"]["qlE"].isArray()) {
				if (OldUserTech["godE"]["qlE"].isArray()) {
					int iold = OldUserTech["godE"]["qlE"][0u].asInt();
					int ichg = updateinfo["tech"]["godE"]["qlE"][0u].asInt();
					int ires = ichg + iold;
					if(ires < 0){
						error_log("[resource_error][uid=%u]", uid);
						LOGIC_ERROR_RETURN_MSG("resource_error");
					}
					updateinfo["tech"]["godE"]["qlE"][0u] = ires;
					if(ichg != 0)
						RESOURCE_LOG("uid=%u,chg=%d,qlE=%d", uid, ichg, ires);
				}
			}
