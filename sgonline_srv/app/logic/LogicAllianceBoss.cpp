...
int CLogicAllianceBoss::GetAllianceBossInfo(unsigned uid, Json::Value &data)
{
	...
	error_log("logic_allianceboss:uid=%u|blood=%u  | fullblood=%u",uid,blood,fullBlood);
...
int CLogicAllianceBoss::Load(unsigned bossId, unsigned uidBy, Json::Value &result)
{
	...
	error_log("logic_allianceboss11:uid=%u|blood=%u  | fullblood=%u",uidBy,blood,fullBlood);
...
int CLogicAllianceBoss::Save(unsigned bossId, DataUser &userBy, Json::Value &data, Json::Value &result)
{
	...
	if(damage > 500000)
	{
		error_log("hurt_is_large: hurt=%u,uid=%u",damage,userBy.uid);
	}
	bool dying = false;
	unsigned blood = 0;
	unsigned number = 0;
