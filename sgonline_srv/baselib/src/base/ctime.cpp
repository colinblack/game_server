...
int CTime::GetWeekInterval(time_t tFrom, time_t tTo)
{
	return ((int)CTime::GetWeekBeginTime(tTo)-(int)CTime::GetWeekBeginTime(tFrom))/CTime::SECONDS_OF_WEEK;
}
...
time_t CTime::GetWeekBeginTime(time_t tTime)
{
	tm tTm = *localtime(&tTime);
	return tTime - tTm.tm_sec - tTm.tm_min*60 - tTm.tm_hour*60*60 - (tTm.tm_wday?tTm.tm_wday-1:6)*CTime::SECONDS_OF_DAY;
}
...
bool CTime::IsDiffDay( time_t t1, time_t t2 )
{
	/*
	// 中国是东八区，比格林威治时间早8小时
	t1 = ( t1 + 8*3600 ) / 86400;
	...
	return t1 != t2;
	*/
	return CTime::GetDayInterval(t1, t2) != 0;
}

bool CTime::IsDiffHour( time_t t1, time_t t2 )
{
	if (IsDiffDay(t1, t2))
	{
		return true;
...
bool CTime::IsDiffWeek( time_t t1, time_t t2 )
{
	return CTime::GetWeekInterval(t1, t2) != 0;
}
