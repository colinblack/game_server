...
CTcpChannel::CTcpChannel() :
	m_channelId(-1), m_isFirst(true),m_bWebSocket(false), m_lastActiveTime(0),m_isFront(true) {
}
...
bool CTcpChannel::IsFirst() const{
	return m_isFirst;
}

void CTcpChannel::SetIsFirst(bool isfirst){
	m_isFirst = isfirst;
}

bool CTcpChannel::IsWebSocket() const{
	return m_bWebSocket;
}

void CTcpChannel::SetIsWebSocket(bool web){
	m_bWebSocket = web;
}
