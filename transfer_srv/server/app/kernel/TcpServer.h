...
#define TCP_SERVER_MAX_CONNECTIONS 100000
...
public:
	...
	virtual int CloseTransferChannel(int channelId);
	virtual bool SendData(CPacket *pHead, IPacketSend *pBody);
	virtual void OnChannelReceive(CTcpChannel *pChannel);

	virtual bool CreateTransferChannel(CTcpChannel *pChannel);
	virtual void OnTransferReceive(int channelId);
...
private:
	...
	map<int, int> m_transfer1;
	map<int, int> m_transfer2;
};
