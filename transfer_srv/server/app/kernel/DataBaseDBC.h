...
#define DBCREQ_EXEC						if ( 0 != req.Execute( m_dbcret ) ){  error_log( "[dbc exec failed][code=%d, msg=%s, from=%s]", m_dbcret.ResultCode(), m_dbcret.ErrorMessage(), m_dbcret.ErrorFrom() ); return m_dbcret.ResultCode(); }
...
#define DBCREQ_SET_INT_V(field)	req.Set(#field, field)
#define DBCREQ_SET_STR_V(field)	req.Set(#field, field.c_str())
#define DBCREQ_ADD_INT_V(field)	req.Add(#field, field)
