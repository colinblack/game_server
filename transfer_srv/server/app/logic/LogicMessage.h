...
public:
	...
	int GetMessage(uint64_t messageId, DataMessage &message);
	...
	int GetChatJson(unsigned uid, int toward, Json::Value &data);
	int RemoveChat(unsigned uid, uint64_t messageId);

	int AddInviteRewardMsg(unsigned uid, const string &name, unsigned inviteUid, int rewardCash);
	int AddInviteUpgradeRewardMsg(unsigned uid, const string &name, unsigned inviteUid, int lvl, int rewardCash);
