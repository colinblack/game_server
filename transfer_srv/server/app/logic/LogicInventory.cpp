...
int CLogicInventory::ChangeItem(unsigned uid, const string &type, int count)
{
	...
	if(ret == 0)
	{
		if(count < 0 && oldCount < (unsigned)-count)
		{
			...
			ERROR_RETURN_MSG(R_ERR_DATA_LIMIT, "not_enough_inventory");
		}
		oldCount += count;
		if(oldCount == 0)
		{
		...
		DATA_INFO_LOG("inventory", "uid=%u&item=%s&delta=%d&count=%u", uid, type.c_str(), count, oldCount);
	}
	else if(ret == R_ERR_NO_DATA)
	{
		...
		DATA_INFO_LOG("inventory", "uid=%u&item=%s&delta=%d&count=%d", uid, type.c_str(), count, count);
	}
	else
	{
