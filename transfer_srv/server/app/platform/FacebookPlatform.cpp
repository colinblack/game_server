...
int CFacebookPlatform::GetUserInfo(OPUserInfo &userInfo, const string &openid, const string &openkey)
{
	...
	{
		m_errorMessage = "response_data_error";
		error_log("[response data error][reason=QueryFail,openid=%s,errmsg=%s,response=%s]",
				openid.c_str(), m_errorMessage.c_str(), response.c_str());
		return -1;
	}
	...
	string email;
	if(Json::GetString(jsonUserInfo, "email", email))
	{
		userInfo.ExtraStrInfo["email"] = email;
	}
...
int CFacebookPlatform::SetScore(const string &openid, const string &openkey, uint64_t score)
{
	string url;
	string response;
	String::Format(url, "https://graph.facebook.com/%s/scores?access_token=%s", openid.c_str(), GetConfig("app_token").c_str());
	string params;
	String::Format(params, "score=%llu", score);
	if(!Network::HttpPostRequest(response, url, params))
	{
		m_errorMessage = response;
		error_log("[http request fail][reason=HttpPostRequest,openid=%s,errmsg=%s]",
	...
	{
		m_errorMessage = response;
		error_log("[response error][reason=QueryFail,openid=%s,errmsg=%s,response=%s]",
				openid.c_str(), m_errorMessage.c_str(), response.c_str());
		return -1;
	}
	return 0;
...
int CFacebookPlatform::SetAchievement(const string &openid, const string &openkey, const string &achievementId)
{
	string url;
	string response;
	String::Format(url, "https://graph.facebook.com/%s/achievements?access_token=%s", openid.c_str(), GetConfig("app_token").c_str());
	string archievement;
	String::Format(archievement, GetConfig("archievement_url").c_str(), achievementId.c_str());
	string params;
	String::Format(params, "achievement=%s", Crypt::UrlEncode(archievement).c_str());
	if(!Network::HttpPostRequest(response, url, params))
	{
		m_errorMessage = response;
		error_log("[http request fail][reason=HttpPostRequest,openid=%s,errmsg=%s]",
