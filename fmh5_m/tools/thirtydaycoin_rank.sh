...
DBNUM=3
LIMITNUM=100
thirtyDayCoinRank="/data/release/sg17/s1/webroot/rankdata/thirtydaycoin_rank.json"

###########################商店近30日金币收入排行#####################
##########获取昨日起始时间戳
MYDATE=$(date -d -30day +%Y%m%d);
STATICTS=$(date -d ${MYDATE} +%s)

##########连表统计
echo '{"rank":[' > ${thirtyDayCoinRank}
count=0
for db in ${HOSTNAMES[@]};
do
    ...
    do
		...
		echo "serverid $dbnum $result" |  awk -F ' ' 'BEGIN{flag=0}{
							if($1=="serverid") {
								print "{"
								print        "\"serverid\":"$2 ","
								print        "\"thirtydaycoin_rank\":["
							}
							else{
								if(flag==1)
									print ","
								print"{\"uid\":"$1 ",\"level\":"$2   ",\"thirtydaycoin\":" $3  ",\"accthumbsup\":"$4 "}"
								flag=1;
							}
						}' >> ${thirtyDayCoinRank} 
		if [ $dbnum == $[DBNUM - 1] ]; then
   			echo ']}' >> ${thirtyDayCoinRank}
		else
    		echo ']},' >> ${thirtyDayCoinRank}
		fi
    done
