...
{
	ProtoFriendOrder::SendOtherServerReq *msg = CreateObj<ProtoFriendOrder::SendOtherServerReq>();
	...
	return ProtoManager::BattleConnectNoReplyByUID(receiverUid, msg);
}
//跨服 接收好友订单
...
int LogicFriendOrderManager::Process(unsigned uid, ProtoFriendOrder::ClickFriendOrderReq *resqmsg)
{
	...
	{
		//跨服查询源订单状态
		ProtoFriendOrder::RecallSourceFoReq *resqmsg = CreateObj<ProtoFriendOrder::RecallSourceFoReq>();
		resqmsg->set_myuid(uid);
		resqmsg->set_myindex(index);
		resqmsg->set_sourceid(sourceId);
		resqmsg->set_senderuid(senderUid);

		return ProtoManager::BattleConnectNoReplyByUID(senderUid, resqmsg);
	}
	else //同服
	{
	...
	ProtoFriendOrder::ClickFriendOrderResp *respmsg = CreateObj<ProtoFriendOrder::ClickFriendOrderResp>();
...
int LogicFriendOrderManager::Process(ProtoFriendOrder::RecallSourceFoReq *resqmsg)
{
	...
	ProtoFriendOrder::ChangeFoStatusReq *msg = CreateObj<ProtoFriendOrder::ChangeFoStatusReq>();
	msg->set_myuid(myuid);
	msg->set_myindex(myindex);
	msg->set_newstatus(newStatus);
	ProtoManager::BattleConnectNoReplyByUID(myuid, msg);
...
int LogicFriendOrderManager::Process(unsigned uid, ProtoFriendOrder::BuyFriendOrderReq *resqmsg)
{
	...
	{
		//跨服查询源订单是否可以购买
		ProtoFriendOrder::RecallCanBuyFoReq *resqmsg = CreateObj<ProtoFriendOrder::RecallCanBuyFoReq>();
		resqmsg->set_myuid(uid);
		resqmsg->set_myindex(index);
		resqmsg->set_sourceid(sourceId);
		resqmsg->set_senderuid(senderUid);
	...
	if (sourceStatus == STATUS_FO_WAIT_BUY) //源订单的状态必须是等待购买
	{
		...
		//---------校验通过
		//物品减少
		ProtoFriendOrder::BuyFriendOrderResp *msg = CreateObj<ProtoFriendOrder::BuyFriendOrderResp>();
...
int LogicFriendOrderManager::Process(ProtoFriendOrder::RecallCanBuyFoReq *resqmsg)
{
	...
	ProtoFriendOrder::AnswerWhetherCanBuyReq *msg = CreateObj<ProtoFriendOrder::AnswerWhetherCanBuyReq>();
