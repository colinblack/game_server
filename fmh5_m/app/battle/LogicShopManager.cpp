...
int LogicShopManager::Process(unsigned uid, ProtoShop::PurchaseReq *req)
{
	...
	{
		//获取商店货架信息
		ProtoShop::CSPurchaseReq *modifyReq = CreateObj<ProtoShop::CSPurchaseReq>();
		...
		//校验通过、发送请求数据
		int ret = ProtoManager::BattleConnectNoReplyByUID(other_uid, modifyReq);
	...
	ProtoShop::PurchaseResp *resp = CreateObj<ProtoShop::PurchaseResp>();
	...
	try
	{
		...
		//推送消息
		ProtoShop::PushShopMsg *msg = CreateObj<ProtoShop::PushShopMsg>();
...
void LogicShopManager::InviteUnlockShopShelf(unsigned uid)
{
	...
	{
		ProtoShop::GetShopResp *msg = CreateObj<ProtoShop::GetShopResp>();
...
int LogicShopManager::Process(ProtoShop::CSPurchaseReq *req)
{
	...
	//发送消息给请求者
	ProtoShop::CSPurchaseResp *resp = CreateObj<ProtoShop::CSPurchaseResp>();
	shop.SetMessage(resp->mutable_visitedshop());
	resp->set_visituid(req->buyeruid());
	resp->set_visiteduid(req->othuid());
	ProtoManager::BattleConnectNoReplyByUID(req->buyeruid(), resp);
...
int LogicShopManager::Process(unsigned uid, ProtoShop::VisitOtherShopReq *req)
{
	...
	{
		ProtoShop::CSVisitOtherShopReq *m = CreateObj<ProtoShop::CSVisitOtherShopReq>();
		m->set_visituid(uid);
		m->set_visiteduid(othuid);
		return ProtoManager::BattleConnectNoReplyByUID(othuid, m);
	}

	ProtoShop::VisitOtherShopResp *resp = CreateObj<ProtoShop::VisitOtherShopResp>();
...
{
	//查询自己的商店信息
	ProtoShop::CSVisitOtherShopResp *resp = CreateObj<ProtoShop::CSVisitOtherShopResp>();
	resp->set_visituid(req->visituid());
	VisitOtherShop(req->visiteduid(), resp->mutable_visitedshopresp());

	return ProtoManager::BattleConnectNoReplyByUID(req->visituid(), resp);
}
