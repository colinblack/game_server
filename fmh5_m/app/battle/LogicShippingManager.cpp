...
int LogicShippingManager::SetShippingboxAidStatus(DataShippingbox &box, int8_t status)
{
	...
	if (box.aid_status == aid_stauts_public || box.aid_status == aid_status_commercial || box.aid_status == aid_stauts_pulic_done || box.aid_status == aid_status_commercial_done)
	{
...
int LogicShippingManager::Process(unsigned uid, ProtoShipping::OfferAidReq *req)
{
	...
	{
		ProtoShipping::OfferAidResp *resp = CreateObj<ProtoShipping::OfferAidResp>();
		...
		ProtoShipping::CSOfferAidReq *m = CreateObj<ProtoShipping::CSOfferAidReq>();
		m->set_othuid(othuid);
		m->set_myuid(uid);
...
int LogicShippingManager::Process(ProtoShipping::CSOfferAidReq *req)
{
	...
	{
		//推送航运箱子
		ProtoShipping::PushShipBox *msg = CreateObj<ProtoShipping::PushShipBox>();
	...
	if (propsid != 0 && propscnt != 0)
	{
		...
		{
			//消息推送
			ProtoShipping::PushFullBoxShipMsg *msg = CreateObj<ProtoShipping::PushFullBoxShipMsg>();
	...
	ProtoShipping::CSOfferAidResp *resp = CreateObj<ProtoShipping::CSOfferAidResp>();
...
int LogicShippingManager::AutoLeaveDock(DataShipping &shipping, ProtoPush::PushBuildingsCPP *msg)
{
	...
	if (is_full_box)
	{
		...
		if (viplevel > 0)
		{
			...
			ProtoShipping::PushShipBonusInfoMsg *msg = CreateObj<ProtoShipping::PushShipBonusInfoMsg>();
