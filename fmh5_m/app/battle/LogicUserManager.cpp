...
OffUserSaveControl::OffUserSaveControl(unsigned uid) : uid_(uid)
{
	//加载用户的玩家档
...
DBCUserBaseWrap::DBCUserBaseWrap(unsigned uid)
	: container_(BaseManager::Instance()), index_(_Index(uid)), data_(container_->m_data->data[index_])
{
}

DBCUserBaseWrap::DBCUserBaseWrap(unsigned index, DataBase &data)
	: container_(BaseManager::Instance()), index_(index), data_(data)
{
}
...
void DBCUserBaseWrap::AddCash(int cash, const std::string &reason)
{
	...
	COINS_LOG("[%s][uid=%u,ocash=%u,ncash=%u,chgcash=%d]", reason.c_str(), data_.uid, data_.cash, data_.cash + cash, cash);
...
void DBCUserBaseWrap::AddCoin(int coin, const std::string &reason)
{
	...
	RESOURCE_LOG("[%s][uid=%u,ocoin=%u,ncoin=%u,chgcoin=%d]", reason.c_str(), data_.uid, data_.coin, data_.coin + coin, coin);
...
void DBCUserBaseWrap::AddAsynItem(unsigned id, unsigned count, const std::string &op)
{
	if ((ASYN_TYPE)id == e_asyn_cash)
	{
		...
		Common::NoticePay *m = CreateObj<Common::NoticePay>();
...
bool DBCUserBaseWrap::AddExp(int exp)
{
	...
	if (exp > 0)
	{
		...
		if (data_.level > old_lvl)
		{
			...
			USER_LOG("[upgrade]uid=%u,open_id=%s,name:%s,old_level=%d,new_level:%d", data_.uid, open_id.c_str(), data_.name, old_lvl, data_.level);
		}
...
void DBCUserBaseWrap::OnUserUpgradeReward(unsigned old_level)
{
	...
	ProtoPush::PushUserUpLevel *msg = CreateObj<ProtoPush::PushUserUpLevel>();
	...
	if (data_.level >= assistCfg.assistinfo().level() && 0 == data_.assist_end_ts)
	{
		...
		ProtoAssistor::OpenAssistorResp *msg_1 = CreateObj<ProtoAssistor::OpenAssistorResp>();
...
void DBCUserBaseWrap::RefreshVIPLevel(bool isPush)
{
	...
	if (data_.viplevel > level)
	{
		...
		//推送VIP等级信息
		ProtoVIP::PushVIPLevel *msg = CreateObj<ProtoVIP::PushVIPLevel>();
...
void LogicUserManager::OnTimer1()
{
	...
	while (!recharge_records_.empty())
	{
		...
		try
		{
			...
			*/
			...
			//推送最新的累积充值数据
			User::PushAccumulateChangeReq *pushmsg = CreateObj<User::PushAccumulateChangeReq>();
	...
	while (!critical_reward_.empty())
	{
		...
		try
		{
			User::CriticalRewardsResp *critical_reward_msg = CreateObj<User::CriticalRewardsResp>();
	...
	while (!level_reward_.empty())
	{
		...
		try
		{
			User::LevelRewardsResp *level_reward_msg = CreateObj<User::LevelRewardsResp>();
	...
	for (uint32_t i = 0; i < ads.reward_size(); ++i)
	{
		...
		if (propsId == item.id())
		{
			if (Math::GetRandomInt(10000) < (100 * item.rate()))
			{
				...
				ProtoReward::FetchProductWatchAdsReward *resp = CreateObj<ProtoReward::FetchProductWatchAdsReward>();
...
int LogicUserManager::UpdateAllianceHelpTimes(unsigned uid, unsigned aid)
{
	...
	{
		ProtoAlliance::RequestAddMemberHelpTimesBC *m = CreateObj<ProtoAlliance::RequestAddMemberHelpTimesBC>();
		m->set_aid(aid);
		m->set_uid(uid);
...
int LogicUserManager::Process(unsigned uid, User::RankThumbsUpReq *req)
{
	...
	{
		User::CSRankThumbsUpReq *m = CreateObj<User::CSRankThumbsUpReq>();
		m->set_othuid(othuid);
		m->set_myuid(uid);
		m->set_index1(index1);
		m->set_index2(index2);
	...
	User::RankThumbsUpResp *resp = CreateObj<User::RankThumbsUpResp>();
	//1.累积被点赞次数
...
int LogicUserManager::Process(User::CSRankThumbsUpReq *req)
{
	...
	User::CSRankThumbsUpResp *resp = CreateObj<User::CSRankThumbsUpResp>();
	resp->set_myuid(req->myuid());
	resp->set_index1(req->index1());
	resp->set_index2(req->index2());
	return ProtoManager::BattleConnectNoReplyByUID(req->myuid(), resp);
}
...
int LogicUserManager::Process(User::CSRankThumbsUpResp *resp)
{
	...
	User::RankThumbsUpResp *msg = CreateObj<User::RankThumbsUpResp>();
	//1.处理点赞奖励
	DBCUserBaseWrap userwrap(uid);
...
int LogicUserManager::PushLoginInfo(unsigned uid, unsigned logints)
{
	...
	if (inviteUid != 0)
	{
		...
		{
			User::CSPushLoginMsg *m = CreateObj<User::CSPushLoginMsg>();
			m->set_inviteuid(inviteUid);
			m->set_logints(logints);
			m->set_uid(uid);
			int ret = ProtoManager::BattleConnectNoReplyByUID(inviteUid, m);
			return ret;
		}
	}
...
int LogicUserManager::HandlePushLoginInfo(unsigned uid, unsigned inviteUid, unsigned loginTs)
{
	...
	{
		User::PushLoginMsg *msg = CreateObj<User::PushLoginMsg>();
