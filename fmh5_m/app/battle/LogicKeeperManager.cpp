...
int LogicKeeperManager::CheckAnimalLogin(unsigned uid, DataAnimal &animal, map<unsigned, set<unsigned>> &tobuild)
{
	...
	for (; animal.full_time <= now && animal.full_time <= keeperOverTs; animal.full_time += spendTime)
	{
		...
		uint32_t taskId = 0;
		// 收获农产品
		if (KeeperCfgWrap().GetProductTaskId(productId, taskId) && LogicKeeperManager::Instance()->CheckKeeperTaskNeed(uid, taskId) && LogicBuildManager::Instance()->GetStorageRestSpace(uid, type_warehouse) > 0) // 助手开启
		{
...
bool LogicKeeperManager::GetKeeperTaskBuildId(uint32_t uid, set<uint32_t> &taskBuild)
{
	...
	for (int i = 0; i < result.size(); i++)
	{
		...
		if (data.need > 0 && data.finish < data.need && IsKeeperOn(uid, GetKeeperIdByTaskId(data.id)) && KeeperCfgWrap().GetProductBuild(data.id, taskBuildId))
		{
				...
				if (userwrap.CheckResBeforeCost(uid, propscfg.material()))
				{
					...
					else
					{
						...
						if (LogicProductLineManager::Instance()->GetFreeProductLine(uid, bId, bUd))
						{
							...
							{
								ProtoProduce::JoinQueueResp *resp = CreateObj<ProtoProduce::JoinQueueResp>();
...
int LogicKeeperManager::OnProductLine(uint32_t uid, uint32_t eud)
{
	...
	*/
	...
	for (set<uint32_t>::iterator iter = productId.begin(); iter != productId.end(); ++iter)
	{
		...
		uint32_t taskId = 0;
		const ConfigItem::PropItem &propscfg = itemcfgwrap.GetPropsItem(pId);
		if (KeeperCfgWrap().GetProductTaskId(pId, taskId) && CheckKeeperTaskNeed(uid, taskId) && userwrap.CheckResBeforeCost(uid, propscfg.material()))
		{
			uint32_t spendTime = LogicProductLineManager::Instance()->GetProduceSpendTime(uid, eud, pId);
			if (spendTime == 0)
			{
...
int LogicKeeperManager::OnAnimalAutoFeed(uint32_t uid)
{
	...
	for (uint32_t i = 0; i < vIdx.size(); ++i)
	{
		...
		ProtoProduce::FeedAnimalResp *feedResp = CreateObj<ProtoProduce::FeedAnimalResp>();
...
int LogicKeeperManager::OnAnimalAutoObtain(uint32_t uid) // 自动收获
{
	...
	for (uint32_t i = 0; i < vIdx.size(); ++i)
	{
		...
		unsigned productId = animalcfg.product().props(0u).id();
		uint32_t taskId = 0;
		if (animal.status != status_full || !KeeperCfgWrap().GetProductTaskId(productId, taskId) || !LogicKeeperManager::Instance()->CheckKeeperTaskNeed(uid, taskId) || LogicBuildManager::Instance()->GetStorageRestSpace(uid, type_warehouse) == 0)
		{
			continue;
		}

		ProtoProduce::ObtainProductResp *obtainResp = CreateObj<ProtoProduce::ObtainProductResp>();
