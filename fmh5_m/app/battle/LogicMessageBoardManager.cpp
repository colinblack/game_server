...
int LogicMessageBoardManager::Process(unsigned uid, ProtoMessageBoard::GetMessageBoardReq *reqmsg)
{
	...
	{
		ProtoMessageBoard::GetMessageBoardResp *respmsg = CreateObj<ProtoMessageBoard::GetMessageBoardResp>();
	...
	else //在别人家访问留言板
	{
		...
		{
			ProtoMessageBoard::GetMasterVisiableMsgReq *reqmsg = CreateObj<ProtoMessageBoard::GetMasterVisiableMsgReq>();
			reqmsg->set_myuid(uid);
			reqmsg->set_masteruid(masteruid);

			ProtoManager::BattleConnectNoReplyByUID(masteruid, reqmsg);
		}
		else //同服
		{
			ProtoMessageBoard::GetMessageBoardResp *respmsg = CreateObj<ProtoMessageBoard::GetMessageBoardResp>();
...
int LogicMessageBoardManager::Process(unsigned uid, ProtoMessageBoard::DeleteMessageBoardReq *reqmsg)
{
	...
	else if (uid != masteruid && uid == senderuid) //在别人家删除自己的留言
	{
		if (CMI->IsNeedConnectByUID(masteruid)) //跨服
		{
			ProtoMessageBoard::DeleteMyMsgOverServerReq *msg = CreateObj<ProtoMessageBoard::DeleteMyMsgOverServerReq>();
			msg->set_myuid(uid);
			msg->set_masteruid(masteruid);
			msg->set_id(msgidx);
			ProtoManager::BattleConnectNoReplyByUID(masteruid, msg);
		}
		else //同服
		{
...
int LogicMessageBoardManager::Process(unsigned uid, ProtoMessageBoard::LeaveMessageReq *reqmsg)
{
	...
	{
		ProtoMessageBoard::SendLeaveMsgOverServerReq *msg = CreateObj<ProtoMessageBoard::SendLeaveMsgOverServerReq>();
		msg->set_model(1);
		msg->set_sender(uid);
		msg->set_receiver(master);
		msg->set_typeid_(typeId);
		msg->set_words(words);
		ProtoManager::BattleConnectNoReplyByUID(master, msg);
	}
	else //同服
	{
		MsgInfoItem msgitem;
		...
		//将刚才的玩家留言推送给前端
		ProtoMessageBoard::LeaveMessageResp *respmsg = CreateObj<ProtoMessageBoard::LeaveMessageResp>();
...
int LogicMessageBoardManager::Process(unsigned uid, ProtoMessageBoard::AnswerLeaveMessageReq *reqmsg)
{
	...
	if (uid != sender) //在自己家给别人留言
	{
		...
		if (CMI->IsNeedConnectByUID(sender)) //跨服
		{
			...
			ProtoManager::BattleConnectNoReplyByUID(sender, msg);
		}
		else //同服
		{
			DynamicInfoAttach *pattach = CreateObj<DynamicInfoAttach>(); //通过动态去创建一条留言
			pattach->op_uid = uid;
...
bool LogicMessageBoardManager::NotifyNewMsg2Client(unsigned uid)
{
	...
	ProtoMessageBoard::HasNewLeaveMessageResp *respmsg = CreateObj<ProtoMessageBoard::HasNewLeaveMessageResp>();
