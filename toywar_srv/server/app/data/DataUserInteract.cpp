...
int CDataUserInteract::AddInteract(const DataUserInteract &interact)
{
	...
	DBCREQ_SET_INT(interact, flag);
	DBCREQ_EXEC;
	return 0;
}

int CDataUserInteract::SetInteract(const DataUserInteract &interact)
{
...
int CDataUserInteract::GetInteract(unsigned uid, unsigned opposite_uid, DataUserInteract &interact)
{
	...
	DBCREQ_GET_INT(interact, flag);

	return 0;
}

int CDataUserInteract::GetInteracts(unsigned uid, vector<DataUserInteract> &interacts)
{
	DBCREQ_DECLARE(DBC::GetRequest, uid);
	DBCREQ_SET_KEY(uid);
	...
	DBCREQ_ARRAY_GET_INT(interacts, flag);
	DBCREQ_ARRAY_GET_END();

	return 0;
}

int CDataUserInteract::GetInteracts(unsigned uid, map<unsigned, DataUserInteract> &interacts)
{
	DBCREQ_DECLARE(DBC::GetRequest, uid);
	DBCREQ_SET_KEY(uid);
...
int CDataUserInteract::GetInteractsAttackAfter(unsigned uid, unsigned last_attack_time, vector<DataUserInteract> &interacts)
{
	DBCREQ_DECLARE(DBC::GetRequest, uid);
	DBCREQ_SET_KEY(uid);
	DBCREQ_SET_CONDITION(GT, last_attack_time, last_attack_time);
...
int CDataUserInteract::RemoveInteracts(unsigned uid)
{
...
int CDataUserInteract::RemoveInteracts(unsigned uid, unsigned opposite_uid)
{
	DBCREQ_DECLARE(DBC::DeleteRequest, uid);
	DBCREQ_SET_KEY(uid);
	DBCREQ_SET_CONDITION(EQ, opposite_uid, opposite_uid);
