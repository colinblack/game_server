...
class CCgiSaveUpdate : public CCgiBase
{

public:

	CCgiSaveUpdate() : CCgiBase("operate")
	{
	...
	int SaveUpdate()
	{
		...
		int result = logicArchive.SaveUpdate(uid, baseid, m_uid, m_data["data"], m_jsonResult);
		if(result != 0)
		{
			return result;
		}
