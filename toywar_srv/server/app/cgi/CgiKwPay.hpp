...
public:
	...
	int KwPay()
	{
		...
		ret = logicPay.BuyCash(uid, cash, credit);
		if(ret != 0)
		{
			error_log("[BuyCash fail][channel=kuaiwan,ret=%d,platform=%d,openid=%s,payid=%s,cash=%u,credit=%u]",
					ret, OpenPlatform::GetType(), openid.c_str(), payid.c_str(), cash, credit);
			return ret;
		}
		payHistory.uid = uid;
