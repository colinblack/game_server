...
{
	//System::SetCurrentDirectory(System::GetModuleDirectory());
	Time::Init();
	Math::InitRandom();
	...
	g_gameType = GT_TOYWAR;
	int gameType;
	if(Config::GetIntValue(gameType, "game"))
	{
		g_gameType = (GameType)gameType;
	}
	return true;
}

GameType Kernel::GetGameType()
{
	return g_gameType;
}
