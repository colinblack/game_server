...
const std::string & html_template::Process()
{
 ...
 in_stream.open( str_tmpl_file_name.c_str() );
...
//reset the html_template state, so that you can use the current obj, it avoids to read template files for many times
void html_template::Reset_State()
{
	variables_c.clear();
	str_tmpl_txt_cpy = str_tmpl_txt;
	bNeedReload = false;
