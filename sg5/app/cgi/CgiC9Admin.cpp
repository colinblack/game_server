...
public:
	...
	int QueryUser()
	{
		...
		result["data"]["last_login"] = user.last_login_time;
		result["data"]["loginlong"] = user.last_active_time - user.last_login_time;
		result["data"]["group"] =  user.kingdom;
		result["data"]["is_valid"] = user.tribute_time > Time::GetGlobalTime() ? 0 : 1;
		result["data"]["exp"] =  (unsigned)user.point;
		result["data"]["createtime"] = user.register_time;
		result["data"]["level"] = user.level;

		string userName;
		CDataUserBasic dbUser;
		dbUser.GetUserName(uid, OpenPlatform::GetType(), userName);
		result["data"]["nickname"] = userName;
