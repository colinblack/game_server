...
CDataMatch::CDataMatch()
{
	m_init = false;
}

int CDataMatch::Init(const string &path, semdat sem)
{
	if(m_init)
	{
		return 0;
	...
	if(!m_sh.CreateOrOpen(path.c_str(), sizeof(MatchData), SEM_ID(sem,semgroup,semserver)))
	{
		error_log("[init_match_fail][path=%s]", path.c_str());
		return R_ERROR;
	}
...
int CDataMatch::View(unsigned aid, unsigned uid, int &stage, MatchTeam &team, MatchRegularRival &rival,
		vector<MatchPlayoffTeam> &playoff)
{
	...
	memset(&team, 0, sizeof(MatchTeam));
	...
	//常规赛的对手
	memset(&rival, 0, sizeof(MatchRegularRival));
...
int CDataMatch::Apply(unsigned aid, const string &aname, int flag, unsigned uid, const string &name)
{
	MatchData *pdata = (MatchData *)m_sh.GetAddress();
	if(pdata == NULL)
	{
	...
	if (apos < pdata->numOfApply)
	{
		...
		if (upos == (pdata->teams[apos]).number && (pdata->teams[apos]).number < MATCH_PLAYERS_MAX)
		{
	...
	else if (pdata->numOfApply < MATCH_TEAMS_MAX)
	{
...
int CDataMatch::ReportResult(unsigned uid, int order, int result)
{
	...
	CAutoLock lock(&m_sh, true);
	if (pdata->stage == MS_REGULAR && order >= 1 && order <= 3)
	{
		for (unsigned i = 0; i < pdata->numOfReg; i++)
		{
			for (unsigned j = 0; j < 5; j++)
			{
				if ((pdata->rteams[i]).rivals[j].player.uid == uid)
				{
					if ((pdata->rteams[i]).rivals[j].result[order-1] == 0)
					{
						(pdata->rteams[i]).rivals[j].result[order-1] = result;
						(pdata->rteams[i]).rivals[j].score += result;
						(pdata->rteams[i]).score += result;
					}
					return 0;
				}
			}
	...
	else if (pdata->stage == MS_PLAYOFF8 && order == 4)
	{
		for (unsigned i = 0; i < 8; i++)
		{
			for (unsigned j = 0; j < 5; j++)
			{
				...
				{
					if ((pdata->top8[i]).result[j] == 0)
					{
						(pdata->top8[i]).result[j] = result;
						(pdata->top8[i]).score += result;
					}
					return 0;
				}
			}
	...
	else if (pdata->stage == MS_PLAYOFF4 && order == 5)
	{
		for (unsigned i = 0; i < 4; i++)
		{
			for (unsigned j = 0; j < 5; j++)
			{
				...
				{
					if ((pdata->top4[i]).result[j] == 0)
					{
						(pdata->top4[i]).result[j] = result;
						(pdata->top4[i]).score += result;
					}
					return 0;
				}
			}
	...
	else if (pdata->stage == MS_PLAYOFF2 && order == 6)
	{
		for (unsigned i = 0; i < 2; i++)
		{
			for (unsigned j = 0; j < 5; j++)
			{
				...
				{
					if ((pdata->top2[i]).result[j] == 0)
					{
						(pdata->top2[i]).result[j] = result;
						(pdata->top2[i]).score += result;
					}
					return 0;
				}
			}
...
int CDataMatch::StartGuess()
{
	...
	CAutoLock lock(&m_sh, true);
	if (pdata->stage != MS_REGULAR)
	{
		return R_ERR_LOGIC;
	}
	pdata->stage = MS_GUESS;
	int rank[8];
	for (int i = 0; i < 8; i++)
		rank[i] = -1;
	for (unsigned i = 0; i < pdata->numOfReg; i++)
	{
		...
		for (; inpos >= 0; inpos--)
		{
			if (rank[inpos] < 0)
				continue;
			if ((pdata->rteams[i]).score <= (pdata->rteams[rank[inpos]]).score)
				break;
		}
		inpos++;
		if (inpos < 8)
...
int CDataMatch::StartPlayoff8()
{
	...
	CAutoLock lock(&m_sh, true);
	if (pdata->stage != MS_GUESS)
	{
		return R_ERR_LOGIC;
	}
	pdata->stage = MS_PLAYOFF8;
	int rank[8];
	for (int i = 0; i < 8; i++)
		rank[i] = -1;
	for (unsigned i = 0; i < pdata->numOfReg; i++)
...
int CDataMatch::StartPlayoff4()
{
	...
	CAutoLock lock(&m_sh, true);
	if (pdata->stage != MS_PLAYOFF8)
	{
		return R_ERR_LOGIC;
	}
	pdata->stage = MS_PLAYOFF4;
	for (unsigned i = 0; i < 4; i++)
	{
		unsigned itemp = 0;
		if ((pdata->top8[2*i]).score < (pdata->top8[2*i+1]).score)
			itemp = 2*i+1;
		else if((pdata->top8[2*i]).score > (pdata->top8[2*i+1]).score){
			itemp = 2*i;
		}
		else{
			CGuessData guessData;
...
int CDataMatch::StartPlayoff2()
{
	...
	CAutoLock lock(&m_sh, true);
	if (pdata->stage != MS_PLAYOFF4)
	{
		return R_ERR_LOGIC;
	}
	pdata->stage = MS_PLAYOFF2;
	for (unsigned i = 0; i < 2; i++)
	{
		unsigned itemp = 0;
		if ((pdata->top4[2*i]).score < (pdata->top4[2*i+1]).score)
			itemp = 2*i+1;
		else if ((pdata->top4[2*i]).score > (pdata->top4[2*i+1]).score)
			itemp = 2*i;
		else{
				CGuessData guessData;
				if(guessData.Init(Config::GetPath(GUESS_DATA_PATH),0) == 0){
...
int CDataMatch::FinishMatch(const MatchData **p)
{
	MatchData *pdata = (MatchData *)m_sh.GetAddress();
	if(pdata == NULL)
	{
...
int CDataMatch::NextTurn()
{
	...
	CAutoLock lock(&m_sh, true);
	//if (pdata->stage != MS_MATCH_FINISH)
	//{
	//	return R_ERR_LOGIC;
	//}
	pdata->stage = MS_APPLY;
	pdata->numOfApply = 0;
	memset(pdata->teams, 0, sizeof(pdata->teams));
	return 0;
}

int CDataMatch::IsMatchProtect(unsigned aid, unsigned uid, bool &prot)
{
	MatchData *pdata = (MatchData *)m_sh.GetAddress();
	if(pdata == NULL)
	{
	...
	prot = false;
	if (pdata->stage < MS_PLAYOFF8 || pdata->stage >= MS_MATCH_FINISH)
	{
		return 0;
	}
	for (unsigned i = 0; i < 8; i++)
...
int CDataMatch::GetApplyPlayersOfTeam(unsigned aid, vector<MatchPlayer> &players)
{
	MatchData *pdata = (MatchData *)m_sh.GetAddress();
	if(pdata == NULL)
	{
...
int CDataMatch::GetRegularScore(unsigned aid, unsigned &score, unsigned &rank, vector<MatchRegularRival> &players)
{
	MatchData *pdata = (MatchData *)m_sh.GetAddress();
	if(pdata == NULL)
	{
	...
	for (unsigned i = 0; i < pdata->numOfReg; i++)
	{
		if ((pdata->rteams[i]).aid == aid)
			continue;
		if ((pdata->rteams[i]).score >= score)
			rank++;
	}
	return 0;
}

int CDataMatch::GetStage(int& stage){
	MatchData *pdata = (MatchData *)m_sh.GetAddress();
	if(pdata == NULL)
	{
		error_log("[GetAddress fail][]");
