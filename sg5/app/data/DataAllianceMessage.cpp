...
int CDataAllianceMessage::AddAllianceMessage(const DataUserMessage &data)
{
...
int CDataAllianceMessage::SetAllianceMessage(const DataUserMessage &data)
{
...
int CDataAllianceMessage::SetAllianceMessageStatus(unsigned uid, uint64_t message_id, int status)
{
...
int CDataAllianceMessage::GetAllianceMessage(unsigned uid, uint64_t message_id, DataUserMessage &data)
{
...
int CDataAllianceMessage::GetAllianceMessage(unsigned uid, int type, int status,
...
int CDataAllianceMessage::GetAllianceMessageForTime(unsigned uid, int type, unsigned startTime, vector<DataUserMessage> &datas)
{
	DBCREQ_DECLARE(DBC::GetRequest, uid);
	DBCREQ_SET_KEY(uid);
	DBCREQ_SET_CONDITION(GT, time, startTime);
...
int CDataAllianceMessage::GetAllianceMessageForTime(unsigned uid, int type, int toward, unsigned startTime, vector<DataUserMessage> &datas)
{
	DBCREQ_DECLARE(DBC::GetRequest, uid);
	DBCREQ_SET_KEY(uid);
	DBCREQ_SET_CONDITION(GT, time, startTime);
