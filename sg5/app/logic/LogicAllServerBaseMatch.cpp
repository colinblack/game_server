...
CDataAllServerBaseMatch* CLogicAllServerBaseMatch::GetAllServerCDataBaseMatch() {
	GET_MEM_DATA_SEM(CDataAllServerBaseMatch, CONFIG_ALL_SERVER_BASE_MATCH_PATH,
			sem_basematchallserver,true)
}

int CLogicAllServerBaseMatch::GetBaseMatchInfo(unsigned aid, unsigned uid,
		Json::Value &data) {
	CDataAllServerBaseMatch *pmatch = GetAllServerCDataBaseMatch();
...
int CLogicAllServerBaseMatch::Apply(unsigned uid) {
	DataUser user;
	...
	CDataAllServerBaseMatch *pbasematch = GetAllServerCDataBaseMatch();
	if (!pbasematch) {
		DB_ERROR_RETURN_MSG("init_basematch_fail");
	}
	unsigned instanceId = 0;
	int serverid = Config::GetZoneByAID(user.alliance_id);
	string allianceName;
...
int CLogicAllServerBaseMatch::ReportResult(unsigned uid, int order, int damage) {
...
bool CLogicAllServerBaseMatch::IsBaseMatchProtect(unsigned aid, unsigned uid) {
	CDataAllServerBaseMatch *pmatch = GetAllServerCDataBaseMatch();
	if (!pmatch)
		return false;
	bool prot = false;
...
int CLogicAllServerBaseMatch::GetApplyPlayers(unsigned aid, Json::Value &result) {
	if (!IsAllianceId(aid)) {
...
int CLogicAllServerBaseMatch::GetRegularScore(unsigned aid, Json::Value &result) {
	if (!IsAllianceId(aid)) {
		PARAM_ERROR_RETURN_MSG("alliance_id_error");
	}
...
int CLogicAllServerBaseMatch::Load(unsigned instid, unsigned uidBy, Json::Value &result) {
	int ret = 0;
	CLogicMatchInstance logicMatchInstance;
...
int CLogicAllServerBaseMatch::Save(unsigned instid, DataUser &userBy,
		Json::Value &data, Json::Value &result, LoadType loadtype) {
	int ret = 0;
	if (Json::IsObject(data, "attackinfo")) {
		CLogicArchive logicArchive;
		ret = logicArchive.ProcessAttackInfo(userBy.uid, data["attackinfo"], result["attackinfo"], instid, loadtype);
		if (ret != 0)
			return ret;
	}
	return 0;
...
int CLogicAllServerBaseMatch::GetStage(int& stage) {
	//CDataBaseMatch *pmatch = GetCDataBaseMatch();
	CDataAllServerBaseMatch *pmatch = GetAllServerCDataBaseMatch();
	if (!pmatch) {
		DB_ERROR_RETURN_MSG("init_basematch_fail");
...
int CLogicAllServerBaseMatch::RequestBaseMatch(const string &url, Json::Value &data, string cgi, bool allserverrank) {
	string serverUrl;
	if(allserverrank)
	{
		if (!Config::GetValue(serverUrl, CONFIG_ALLSERVERRANK_PATH))
		{
		...
		if(serverUrl[serverUrl.length() - 1] != '/')
			serverUrl.append("/");
		serverUrl.append(cgi);
	}
	else
	{
		serverUrl = Config::GetAllServerUrl(cgi);
		if (serverUrl.empty()) {
			error_log("[get config fail][path=]");
			return R_ERR_DATA;
		}
...
//		response.clear();
	...
	if (Json::GetString(data, "error", errorCode) && "0" != errorCode)
	{
		...
		DATA_ERROR_RETURN_MSG(errorCode);
	}
	return 0;
}
