...
int CLogicCustomServiceAdmin::CopyArchive(unsigned fromUid, unsigned toUid)
{
	CLogicAdmin logicAdmin;
	Json::Value data;
	int ret = logicAdmin.ExportArchive(fromUid,data);
	if (0 != ret)
	{
		return ret;
	}
	ret = logicAdmin.ImportArchive(toUid, data);
	if (0 != ret)
	{
		return ret;
	}
