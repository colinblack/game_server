...
int C9Platform::GetUserInfo(OPUserInfo &userInfo, const string &openid, const string &openkey)
{
	...
	if(GetPlatformType() == PT_FENGHUANG)
	{
		string url = "http://play.ifeng.com/?_c=act&_a=getuservip&uid=" + openid + "&game=zzby&sign=" + Crypt::Md5Encode(openid + "zzbydvthHG3%FgvL*nfg");
		string response;
		Json::Value value;
		try
		{
			if (Network::HttpGetRequest(response, url) && !response.empty() && Json::Reader().parse(response, value)
			&& value.isMember("suc") && value["suc"].asInt() == 1 && value.isMember("vip"))
				userInfo.ExtraIntInfo["fh_vip_level"] = CTrans::STOI(value["vip"].asString());
			else
				userInfo.ExtraIntInfo["fh_vip_level"] = 0;
		}
