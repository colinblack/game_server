...
int UserManager::LoadArchives(unsigned uid)
{
	...
	try
	{
		...
		//英雄
		DataHeroManager::Instance()->LoadBuffer(uid);
		//关注的人
		DataConcernManager::Instance()->LoadBuffer(uid);
		//武馆
		DataMartialClubManager::Instance()->LoadBuffer(uid);
		//武馆中的英雄
		DataMartialHeroManager::Instance()->LoadBuffer(uid);
		//邀请玩家中的信息
		DataInviteManager::Instance()->LoadBuffer(uid);
		//兼职武馆英雄
		DataDetachHeroManager::Instance()->LoadBuffer(uid);
		//卡牌
		DataCardsManager::Instance()->LoadBuffer(uid);
	}
...
/**************下面是写登录返回包的数据，数据处理应该在此前完成************************/
	...
	reply->set_drawrewardid(LUI->GetDrawRewardId(uid));
	...
	try
	{
		...
		//英雄
		LogicHeroManager::Instance()->FullMessage(uid, reply->mutable_heros());

		//卡牌
		LogicCardsManager::Instance()->FullMessage(uid, reply->mutable_cards());

		//已解锁卡牌
		LogicCardsManager::Instance()->FullMessage(uid, reply->mutable_unlock());
		//排位赛
		LogicUserGradeManager::Instance()->FullMessage(uid, reply->mutable_grade());

		//朋友
		LogicFriendManager::Instance()->FullMessage(uid, reply->mutable_friends());

		//离线挂机奖励
		LogicOnhookManager::Instance()->FullMessage(uid, reply->mutable_onhook());

		//武馆
		LogicMartialClubManager::Instance()->FullMessage(uid, reply->mutable_allclub());
		//通知
		LogicPostManager::Instance()->FullMessage(uid, reply->mutable_post());

		//邀请好友
		DataInviteManager::Instance()->FullMessage(uid, reply->mutable_inviteinfo());
	}
	catch(const std::exception& e)
	{
		if (reply)
...
int UserManager::OnNewUser(unsigned uid, Common::Login* msg)
{
	...
	base.recover_time = Time::GetGlobalTime();
	base.onhook_coin_ts = Time::GetGlobalTime();
	base.onhook_exp_ts = Time::GetGlobalTime();
	...
	debug_log("newuser name : %s", base.name);
	const UserCfg::UserBase& userBaseCfg = UserCfgWrap().UserBase();
	auto& userCfg = UserCfgWrap().User();
	base.level = userBaseCfg.level();
	base.exp = userBaseCfg.exp();
	//todo: new user init
	base.coin = userBaseCfg.coin();
	base.cash = userBaseCfg.cash();
	base.power = POWER_MAX;
	base.heroId =  userCfg.hero(0).id();
	BaseManager::Instance()->m_data->MarkChange(index);	

	try
	{
		//初始英雄
		for(int i = 0; i < userCfg.hero_size(); ++i)
			LogicHeroManager::Instance()->AddHeros(uid, userCfg.hero(i).id(), "new_usr_init_hero");

		//初始卡牌
		for(int i = 0; i < userCfg.cards_size(); ++i)
			LogicCardsManager::Instance()->AddCards(uid, userCfg.cards(i).id(), "new_user_card", nullptr);

		//添加系统好友
		DataConcernManager::Instance()->GetData(uid, uid);
		//系统好友赠送体力
		LogicUserManager::Instance()->DonatePower(uid, POWER_DONATE_PER, "sys_friend_donate_power");
		//道具
		LogicPropsManager::Instance()->NewUser(uid);

		OfflineResourceItem& item = LogicResourceManager::Instance()->Get(uid);
		item.last_login_time = base.last_login_time;
	}
	catch (const std::exception& e)
	{
		error_log("uid:%u, %s", uid, e.what());
...
int UserManager::OnUserLogin(unsigned uid, Common::Login* msg)
{
	...
	try
	{
		...
		LogicUserGradeManager::Instance()->Get(uid);
		DBCUserBaseWrap user(index, base);
		user.RecoverPower(Time::GetGlobalTime(), base.recover_time);
		user.FinalAsynData();
		...
		//离线武馆收益
		LogicMartialClubManager::Instance()->MartialOfflineIncome(uid, base.last_off_time, base.last_login_time);
		//更新战力
		LogicHeroManager::Instance()->UpdateBattlePower(uid);
...
int UserManager::UserOffLine(unsigned uid)
{
	...
	//todo: offline
	unsigned tmp 			= base.last_off_time;
	...
	try
	{
		LogicUserManager::Instance()->ResetInviteRwards(uid, tmp);
		LogicResourceManager::Instance()->Offline(uid);
		//更新武馆收益
		LogicMartialClubManager::Instance()->ALLMartialClubIncome(uid);
		//更新武馆英雄信息
		LogicMartialHeroManager::Instance()->UpdateMartialHeroInfo(uid);
	}
	catch(const std::exception& e)
	{
		error_log("uid:%u, %s", uid, e.what());
