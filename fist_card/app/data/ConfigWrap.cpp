...
const Hero::CardByGetHero& HeroCfgWrap::GetCardsByHeroId(unsigned heroId)
{
	if(!ConfigManager::Instance()->hero_cards_id_index.count(heroId))
	{
		error_log("param error");
		throw runtime_error("param_error");
	}

	unsigned index = ConfigManager::Instance()->hero_cards_id_index[heroId];
	return cfg_.cards(index);
}
...
CardsCfgWrap::CardsCfgWrap()
	:cfg_(ConfigManager::Instance()->cardsCfg.m_config)
{
}

const Cards::CardInfo& CardsCfgWrap::GetCardsCfgById(unsigned id)
{
	if(!ConfigManager::Instance()->card_id_index.count(id))
	{
		error_log("param error: %u", id);
		throw runtime_error("param_error");
	}

	unsigned index = ConfigManager::Instance()->card_id_index[id];
	return cfg_.cards(index);
}

const Cards::CardsCfg& CardsCfgWrap::Obj()
{
	return cfg_;
}

const Cards::CardUpdateStar& CardsCfgWrap::GetStartUpdateCfgById(unsigned id)
{
	if(!ConfigManager::Instance()->card_update_id_index.count(id))
	{
		error_log("param error");
		throw runtime_error("param_error");
	}

	unsigned index = ConfigManager::Instance()->card_update_id_index[id];
	return cfg_.update(index);
}


const Cards::CardDup& CardsCfgWrap::GetStartDupCfgById(unsigned id)
{
	if(!ConfigManager::Instance()->card_dup_id_index.count(id))
	{
		error_log("param error");
		throw runtime_error("param_error");
	}

	unsigned index = ConfigManager::Instance()->card_dup_id_index[id];
	return cfg_.dup(index);
}

const Cards::CardPurchase& CardsCfgWrap::GetPurchaseCfgById(unsigned id)
{
	if(!ConfigManager::Instance()->card_purchase_id_index.count(id))
	{
		error_log("param error");
		throw runtime_error("param_error");
	}

	unsigned index = ConfigManager::Instance()->card_purchase_id_index[id];
	return cfg_.buy(index);
}
