...
void ConfigManager::Init()
{
    ...
    card_id_index.clear();
    for(int i = 0; i < cardsCfg.m_config.cards_size(); ++i)
    {
    	unsigned id = cardsCfg.m_config.cards(i).id();
    	card_id_index[id] = i;
    }

    card_update_id_index.clear();
    for(int i = 0; i < cardsCfg.m_config.update_size(); ++i)
    {
    	unsigned id = cardsCfg.m_config.update(i).id();
    	card_update_id_index[id] = i;
    }
    card_dup_id_index.clear();
    for(int i = 0; i < cardsCfg.m_config.dup_size(); ++i)
    {
    	unsigned id = cardsCfg.m_config.dup(i).id();
    	card_dup_id_index[id] = i;
    }

    card_purchase_id_index.clear();
    for(int i = 0; i < cardsCfg.m_config.buy_size(); ++i)
    {
    	unsigned id = cardsCfg.m_config.dup(i).id();
    	card_purchase_id_index[id] = i;
    }

    hero_cards_id_index.clear();
    for(int i = 0; i < hero.m_config.cards_size(); ++i)
    {
    	unsigned id = hero.m_config.cards(i).heroid();
    	hero_cards_id_index[id] = i;
    }
