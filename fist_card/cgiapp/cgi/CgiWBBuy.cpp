...
public:
	...
	int buy()
	{
		...
		if (openid.empty() || openkey.empty() || pf.empty() || count == 0 || id.empty() || platform.empty())
		{
			PARAM_ERROR_RETURN_MSG("param_error");
		}
		map<string, string> pt_param;
		pt_param["pt"] = platform;
		pt_param["pf"] = pf;
		OpenPlatform::GetPlatform()->SetParameter(pt_param);
		int pt = OpenPlatform::GetPlatform()->GetPlatformByPF();
		if(pt != PT_WB_AND && pt != PT_WB_IOS)
		{
			PARAM_ERROR_RETURN_MSG("param_error");
		}

		unsigned uid = 0;
		int ret = CDataUserMapping().GetMapping(openid, pt, uid);
		if(ret)
		{
		...
		QQItemInfo itemInfo;
		ret = CLogicQQPay::GetItemInfo(id, itemInfo, CTrans::STOI(platform));
		if (ret)
			return ret;
		...
		param_map["zoneid"] = platform;
		param_map["billno"] = Crypt::Md5Encode(openid + CTrans::ITOS(Time::GetGlobalTime()) + itemInfo.pid);
		param_map["itemid"] = itemInfo.pid;
