...
int CDataUserMapping::GetMapping(const string &openid, int platform, unsigned &uid)
{
	...
	*/
	...
	if(ConfigManager::Instance()->GetServer())
		return GetUID(openid, ConfigManager::Instance()->GetServer(), platform, uid);

	vector<unsigned> uids;
	int ret = GetMultiplePlatform(openid, platform, uids);
	if(ret)
		return ret;
	if(uids.size() == 0)
	{
...
int CDataUserMapping::GetMultiplePlatform(const string &openid, int platform, vector<unsigned> &uid) {
	unsigned crc = CCRC::GetCrc32(openid);
	if (crc > UID_MAX) {
		crc %= UID_MAX;
		if (crc < UID_MIN) {
...
int CDataUserMapping::GetUID(const string &openid, unsigned serverid, int platform, unsigned &uid){
	unsigned crc = CCRC::GetCrc32(openid);
	if (crc > UID_MAX) {
		crc %= UID_MAX;
		if (crc < UID_MIN) {
...
int CDataUserMapping::GetServer(const string &openid, int platform, vector<server> &servers)
{
	unsigned crc = CCRC::GetCrc32(openid);
	if (crc > UID_MAX) {
		crc %= UID_MAX;
