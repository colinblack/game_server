...
public:
	...
	int GetMultiplePlatform(const string &openid, int platform, vector<unsigned> &uid);
	int GetAllRegisterTime(const string &openid, vector<unsigned> &registertime);
	int GetUID(const string &openid, unsigned serverid, int platform, unsigned &uid);
	...
	int GetServer(const string &openid, int platform, vector<server> &servers);
